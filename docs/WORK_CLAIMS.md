...
| `rules-counter-then-controller-token` | `worker-05` | New compound effect `counter-target-spell-then-controller-token` (An Offer You Can't Refuse): counters a spell, then its (former, since it's about to leave the stack) controller — not the caster — creates N tokens, resolved from `state.stack.find(...).controller` before the entry is marked `countered`. Modeled on the existing `destroy-target-creature-then-controller-token` precedent, but that one hardcodes a single token; this one threads `amount` through since Treasure comes in twos. Reuses `parseCreateToken` and the existing `noncreature-spell` target kind. Fully implements An Offer You Can't Refuse. LIMIT: Swan Song and Strix Serenade use the identical "counter + controller token" shape but with different spell-type filters ("enchantment, instant, or sorcery spell" / "artifact, creature, or planeswalker spell") that don't map to any existing `TargetKind`; left for a follow-up that adds those filters | Ready for integrator review |
| `rules-trigger-doubler` | `worker-05` | New reusable "Panharmonicon" primitive (CR 603.3f): "If a triggered ability of X triggers, that ability triggers an additional time" (Harmonic Prodigy, Wizard's Staff, and 10 other catalog cards with different scope filters). New `CardProfile.triggerDoublers: readonly TriggerDoubler[]` supporting the two shapes actually needed — `subtype-you-control` (a list of subtypes; matches if the triggering permanent has any of them) and `equipped-creature` — checked in `raiseEvent` (`engine.ts`) via a new `triggerDoublerCount` helper that queues `1 + count` copies of the same `TriggerInstance` (with distinct ids) instead of one whenever a matching doubler is on the controller's battlefield. Fully implements Harmonic Prodigy and (as a byproduct — same "single subtype, you control" shape) Katara, the Fearless. LIMIT: Wizard's Staff still needs its separate "Equip Wizard {1}" typed-equip-cost clause (a different, unrelated primitive) to reach `fullyImplemented`; Delney, Streetwise Lookout and most of the other 9 catalog cards use scope filters (power threshold, ownership-not-control, legendary, color, "another Wolf or battle") this claim doesn't model — only `subtype-you-control` and `equipped-creature` are wired | Ready for integrator review |
| `rules-typed-equip-cost` | `worker-05` | New reusable primitive closing out Wizard's Staff: a typed Equip ability ("Equip Wizard {1}") alongside the card's own general "Equip {3}" — a second, usually cheaper, Equip cost restricted to a creature subtype (CR 702.6e-style templating). New `CardProfile.typedEquipCost: { subtype, cost } \| null`, parsed by a `parseTypedEquipCost` sibling to the existing `parseEquipCost` (which already skips the typed line safely, since `parseManaCost("Wizard {1}")` fails and the loop falls through to the general "Equip {3}" line). `applyEquip` now picks whichever cost applies to the actually-chosen target creature (checked via `hasSubtype`), and `legalActions`' equip-offering check accepts either cost being affordable. Fully implements Wizard's Staff plus 8 other catalog cards using the same "Equip \<Subtype\> {cost}" / "Equip {cost}" pairing: Commander's Plate, Unstable Molecule Suit, Dúnedain Blade, Thinking Cap, Pirate Hat, Veteran's Powerblade, Ceremonial Groundbreaker, Steelclaw Lance | Ready for integrator review |
| `rules-standstill` | `worker-05` | New compound trigger effect for Standstill: "When a player casts a spell, sacrifice ~. If you do, each of that player's opponents draws three cards." Two small gaps, one card: (1) the `each-player` spell-cast `TRIGGER_TEMPLATES` entry required literal "Whenever", but Standstill's own Oracle text uses "When" (correct real-world templating for an ability that can only fire once) — widened to accept either; (2) new `each-opponent-of-event-player-draws` effect, scoped to `object.trigger.eventController` (the event's own caster, already captured generically from any `controller`-keyed event) rather than the ability's controller, combined with the existing `sacrifice-source` via `compound`. Fully implements Standstill | Ready for integrator review |
| `rules-instant-or-sorcery-cast-each-player-draw` | `worker-05` | New `TRIGGER_TEMPLATES` entry: "Whenever a player casts an instant or sorcery spell, ~" (`event: "spell-cast"`, `subject: "each-player"`, `spellType: "instant-or-sorcery"`) — the exact same shape already added twice this session for `spellType: "creature"` and `"noncreature"`, just never done for `"instant-or-sorcery"` with the `each-player` subject. No new effect needed: "you draw a card" already resolves via the existing `draw` effect off the ability's own controller. Fully implements Niv-Mizzet, Parun (its sibling clause "Whenever you draw a card, ~ deals 1 damage to any target" was already modeled, so the two now chain into the card's real combo without any further change) | Ready for integrator review |
