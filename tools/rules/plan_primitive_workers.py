...
def build_worker_plan(
    ...
    *,
    ...
    near_complete_ids: set[str] | None = None,
) -> dict[str, Any]:
    ...
    for entry in roadmap:
        ...
        card_payload = entry.get("affected_cards") or entry.get("unlocked_cards") or entry.get("cards") or []
        review_cards = [
            card for card in card_payload
            if isinstance(card, dict)
            and str(card.get("oracle_id") or "") in (oracle_review_ids or set())
        ]
        one_line_cards = entry.get("one_line_cards") or []
        near_complete_cards = [
            card for card in card_payload
            if isinstance(card, dict) and str(card.get("oracle_id") or "") in (near_complete_ids or set())
        ]
        ...
        jobs.append(
            {
                ...
                "needs_review_count": len(review_cards),
                ...
                "near_complete_count": len(near_complete_cards),
                "near_complete_cards": near_complete_cards,
                "semantic_atoms": semantic_atoms,
                "priority": str(entry.get("priority") or (
                    "needs-review" if review_cards else
                    "quick-win" if entry.get("quick_win_count") or one_line_cards or near_complete_cards else
                    "normal"
                )),
                "oracle_ids": cards,
                "oracle_cards": card_payload,
                "batches": batches,
            }
        )
        ...
        work_units.append(
            {
                ...
                "needs_review_count": sum(job["needs_review_count"] for job in component_jobs),
                "one_line_count": sum(job["one_line_count"] for job in component_jobs),
                "unlocks": sum(job["unlocks"] for job in component_jobs),
                "claim_key": min(job["claim_key"] for job in component_jobs),
            }
        )
    # Review-first is a scheduling signal, not a rules claim. Resolve explicit
    # Oracle needs-review cards first, then exact one-line candidates, then
    # broader ROI. This maximizes cards closed per minute without guessing.
    work_units.sort(key=lambda unit: (
        -unit["needs_review_count"],
        -unit["one_line_count"],
        -unit["quick_win_count"],
        -len(unit["oracle_ids"]),
        -unit["unlocks"],
        unit["claim_key"],
    ))
    ...
    return {
        ...
        "selection_policy": "review-first-random-unclaimed-with-recheck",
        ...
        "near_complete_source": "engine-card-profiles.json exact one-line export" if near_complete_ids is not None else None,
...
def render_document(plan: dict[str, Any]) -> str:
    ...
    for worker in plan["workers"]:
        ...
        if not worker["jobs"]:
            ...
            lines.append("")
            continue
        lines += ["| Priority | Claim | Family | Cards | Needs review | One-line | Reuse queue | Unlocks | Batches | Atoms |", "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |"]
        for job in worker["jobs"]:
            lines.append(
                f"| {job['priority']} | `{job['claim_key']}` | {job['family']} | {len(job['oracle_ids'])} |"
                f" {job['needs_review_count']} | {job['one_line_count']} | {job.get('near_complete_count', 0)} | {job['unlocks']} | {len(job['batches'])} | {len(job.get('semantic_atoms', []))} |"
            )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional identify_near_complete_cards JSON; promotes exact one-line cards in the worker plan.",
    )
    args = parser.parse_args()
    payload = json.loads(args.roadmap.read_text(encoding="utf-8"))
    oracle_review_ids: set[str] | None = None
    if args.oracle_effects and args.oracle_effects.exists():
        oracle_payload = json.loads(args.oracle_effects.read_text(encoding="utf-8"))
        oracle_review_ids = {
    ...
    near_complete_ids: set[str] | None = None
    if args.near_complete:
        if not args.near_complete.exists():
            raise SystemExit(f"Near-complete index not found: {args.near_complete}")
        near_payload = json.loads(args.near_complete.read_text(encoding="utf-8"))
        near_complete_ids = {
            str(card.get("oracle_id"))
            for card in near_payload.get("cards", [])
    ...
    plan = build_worker_plan(
        ...
        review_payload_mode=review_payload_mode,
        near_complete_ids=near_complete_ids,
    )
