...
const C13_CHAOS_WARP = () => ({ ...CHAOS_WARP(), scryfall_id: "07a0cba9-8768-4fd9-a3d5-b0f83b4bf8e8" });
...
describe("casting", () => {
  ...
  it("reuses oracle-driven rules for C13 reprints", () => {
    expect(C13_CHAOS_WARP().scryfall_id).toBe("07a0cba9-8768-4fd9-a3d5-b0f83b4bf8e8");
    ...
    expect(cardProfile(C13_CHAOS_WARP()).fullyImplemented).toBe(true);
  ...
  it("offers and pays an activated counter-removal cost", () => {
    ...
    const activation = legalActions(game, 0).find((entry) => entry.action.type === "activate" && entry.action.sourceId === source.instance_id);
    expect(activation).toBeDefined();
    if (!activation || activation.action.type !== "activate") throw new Error("Expected an activation action.");
