...
describe("Ward keyword", () => {
  function wardGame(): GameState {
    ...
    game = stage(game, 0, () => ({
      hand: toHand(0, [WARD_TARGET_SPELL()]),
      manaPool: { W: 0, U: 0, B: 0, R: 1, G: 0, C: 2 }
    }));
    return putOnBattlefield(game, 1, [WARD_SENTINEL()]);
  }

  it("recognises the cost and counters an opposing spell when unpaid", () => {
    expect(cardProfile(WARD_SENTINEL()).wardCost?.raw).toBe("{2}");
    let game = wardGame();
    const ward = game.players[1]!.battlefield[0]!;
    game = applyAction(game, 0, { type: "cast", cardId: "hand-0", targets: [{ kind: "permanent", instanceId: ward.instance_id }] });
  ...
  it("lets the opposing caster pay Ward and keeps the spell on the stack", () => {
    let game = wardGame();
    const ward = game.players[1]!.battlefield[0]!;
    game = applyAction(game, 0, { type: "cast", cardId: "hand-0", targets: [{ kind: "permanent", instanceId: ward.instance_id }] });
    const choice = game.pendingChoice!;
...
const SPRITE_DRAGON = () => make({ name: "Sprite Dragon", type_line: "Creature — Faerie Dragon", mana_cost: "{U}{R}", cmc: 2, power: "1", toughness: "1", keywords: ["Flying", "Haste"], oracle_text: "Flying, haste\nWhenever you cast a noncreature spell, put a +1/+1 counter on this creature.", oracle_id: "a9d8ab76-70a4-475e-b87e-4737c090553a", scryfall_id: "c9f2c4ac-2e2b-4f15-bb9c-8ca54e4c3fbf" });
const MISTMEADOW_WITCH = () => make({ name: "Mistmeadow Witch", type_line: "Creature — Kithkin Wizard", mana_cost: "{2}{W}{U}", cmc: 4, power: "1", toughness: "3", oracle_text: "{2}{W}{U}: Exile target creature. Return that card to the battlefield under its owner's control at the beginning of the next end step.", oracle_id: "38e274e2-bd04-48de-a1df-44f0ee987ba8", scryfall_id: "aeb05bcc-345e-4883-9b43-1c3cfa3a4f51" });
const ROON_OF_THE_HIDDEN_REALM = () => make({ name: "Roon of the Hidden Realm", type_line: "Legendary Creature — Rhino Soldier", mana_cost: "{2}{G}{W}{U}", cmc: 5, power: "4", toughness: "4", keywords: ["Vigilance", "Trample"], oracle_text: "Vigilance, trample\n{2}, {T}: Exile another target creature. Return that card to the battlefield under its owner's control at the beginning of the next end step.", oracle_id: "fd336830-4a11-42b8-9fc7-d7526f569124", scryfall_id: "3045053c-9a90-4caa-9096-9256bf007d3a" });
...
describe("casting", () => {
  ...
  it("reuses source-counter thresholds for C13 Plague Boiler", () => {
    const profile = profileOf(C13_PLAGUE_BOILER());
    expect(profile.activatedAbilities[0]).toMatchObject({
      manaCost: { raw: "{1}{B}{G}" }, effect: { kind: "toggle-source-counter", counter: "plague" }
    });
    ...
    let game = readyToCast([], [C13_PLAGUE_BOILER(), MOUNTAIN(), SWAMP(), FOREST()], [], [BEAR()]);
    game = stage(game, 0, (player) => ({
      battlefield: player.battlefield.map((permanent) => permanent.card.name === "Plague Boiler"
        ? { ...permanent, counters: { plague: 2 } } : permanent)
    }));
    const boiler = game.players[0]!.battlefield.find((permanent) => permanent.card.name === "Plague Boiler")!;
    game = applyAction(game, 0, { type: "activate", sourceId: boiler.instance_id, abilityIndex: 0 });
  ...
  it("puts mana-spent counters on C13 Marath as it enters", () => {
    expect(profileOf(C13_MARATH())).toMatchObject({ entersWithSpentManaCounters: true });
    let game = readyToCast([C13_MARATH()], [FOREST(), MOUNTAIN(), PLAINS(), FOREST(), FOREST()]);
  ...
  it("reuses the sacrificed-creature event for Thraximundar", () => {
    expect(profileOf(C13_THRAXIMUNDAR())).toMatchObject({
      fullyImplemented: true,
      triggers: [
        { event: "attacks", effect: { kind: "defending-player-sacrifice-creature" } },
        { event: "permanent-sacrificed", subject: "any-creature", optional: true, effect: { kind: "add-counter-source", counter: "+1/+1", amount: 1 } }
      ]
    });
    let game = readyToCast([BOLT()], [C13_THRAXIMUNDAR(), CARNAGE_ALTAR(), BEAR(), MOUNTAIN(), MOUNTAIN(), MOUNTAIN()]);
    const altar = game.players[0]!.battlefield.find((permanent) => permanent.card.name === "Carnage Memory")!;
    const bear = game.players[0]!.battlefield.find((permanent) => permanent.card.name === "Grizzly Bears")!;
    const activation = legalActions(game, 0).find((entry) => entry.action.type === "activate" && entry.action.sourceId === altar.instance_id)!;
    if (activation.action.type !== "activate") throw new Error("Carnage Altar sacrifice activation was not offered.");
  ...
  it("pays Uyo's two-land return cost before copying a spell", () => {
    ...
    let game = readyToCast([AZORIUS_SPELL()], [C13_UYO(), PLAINS(), ISLAND(), ISLAND(), ISLAND(), ISLAND(), ISLAND(), ISLAND()]);
    game = applyAction(game, 0, { type: "cast", cardId: "hand-0" });
    const original = game.stack.at(-1)!;
    const activation = legalActions(game, 0).find((entry) => entry.action.type === "activate"
      && entry.action.sourceId === game.players[0]!.battlefield.find((permanent) => permanent.card.name === "Uyo, Silent Prophet")!.instance_id)!;
    if (activation.action.type !== "activate") throw new Error("Uyo activation was not offered.");
    game = applyAction(game, 0, { ...activation.action, targets: [{ kind: "spell", stackId: original.id }] });
    expect(game.players[0]!.battlefield.filter((permanent) => permanent.card.name === "Island")).toHaveLength(5);
  });
  ...
  it("applies a source pump while an opponent is at or below its life threshold", () => {
    expect(profileOf(RUTHLESS_CULLBLADE()).staticPowerToughnessGrants).toEqual([
      { scope: "source-opponent-life-threshold", power: 2, toughness: 1, threshold: 10 }
    ]);
    let game = twoSeatGame([], []);
    game = putOnBattlefield(game, 0, [RUTHLESS_CULLBLADE()]);
    const cullblade = game.players[0]!.battlefield[0]!;
    expect([powerOf(cullblade, game), toughnessOf(cullblade, game)]).toEqual([2, 1]);
    game = stage(game, 1, () => ({ life: 10 }));
    expect([powerOf(cullblade, game), toughnessOf(cullblade, game)]).toEqual([4, 2]);
    game = stage(game, 1, () => ({ life: 11 }));
    expect([powerOf(cullblade, game), toughnessOf(cullblade, game)]).toEqual([2, 1]);
  });

  it("applies a source pump while its controller controls the required basic land subtype", () => {
    expect(profileOf(LOAM_LION()).staticPowerToughnessGrants).toEqual([
      { scope: "source-control-land-subtype", power: 1, toughness: 2, subtype: "Forest" }
    ]);
    let game = twoSeatGame([], []);
    game = putOnBattlefield(game, 0, [LOAM_LION()]);
    const lion = game.players[0]!.battlefield[0]!;
  ...
  it("reuses compound damage and life gain for a creature-only target", () => {
    expect(profileOf(ESSENCE_EXTRACTION())).toMatchObject({
      targetKind: "creature",
      effects: [{ kind: "compound", effects: [{ kind: "damage-any-target", amount: 3 }, { kind: "gain-life", amount: 3 }] }],
      fullyImplemented: true
    });
    let game = readyToCast([ESSENCE_EXTRACTION()], [SWAMP(), SWAMP(), SWAMP()], [], [BEAR()]);
    const bear = game.players[1]!.battlefield.find((permanent) => permanent.card.name === "Grizzly Bears")!;
    game = applyAction(game, 0, { type: "cast", cardId: "hand-0", targets: [{ kind: "permanent", instanceId: bear.instance_id }] });
    expect(game.players[0]!.life).toBe(43);
    expect(game.players[1]!.battlefield.some((permanent) => permanent.instance_id === bear.instance_id)).toBe(false);
  ...
  it("strips ability-word labels before parsing an enters trigger", () => {
    expect(profileOf(RED_DRAGON())).toMatchObject({
      triggers: [{ event: "enters-battlefield", subject: "self", effect: { kind: "damage-each-opponent", amount: 4 }, targetKind: "none" }],
      fullyImplemented: true
    });
  ...
  it("stores True-Name Nemesis's chosen player and applies protection to targeting", () => {
    const profile = profileOf(C13_TRUE_NAME());
    expect(profile).toMatchObject({
      fullyImplemented: true,
      triggers: [{ event: "enters-battlefield", effect: { kind: "set-source-protection-from-player" }, targetKind: "player" }]
    });
    let game = readyToCast([C13_TRUE_NAME()], [ISLAND(), ISLAND(), ISLAND()]);
    game = applyAction(game, 0, { type: "cast", cardId: "hand-0" });
    game = passUntil(game, (state) => state.pendingChoice?.type === "trigger-target");
    const choice = game.pendingChoice as Extract<GameState["pendingChoice"], { type: "trigger-target" }>;
    game = applyAction(game, 0, { type: "choose-trigger-target", sourceId: choice.sourceId, target: { kind: "player", seat: 1 } });
    game = passUntil(game, (state) => state.stack.length === 0 && state.pendingChoice === null);
    const nemesis = game.players[0]!.battlefield.find((permanent) => permanent.card.name === "True-Name Nemesis")!;
    expect(nemesis.protectionFromPlayers).toEqual([1]);
    const removal = make({ name: "Test Removal", type_line: "Instant", mana_cost: "{B}", oracle_text: "Destroy target creature." });
    expect(legalTargets(game, 1, "creature", profileOf(removal))).not.toContainEqual({ kind: "permanent", instanceId: nemesis.instance_id });
    expect(legalTargets(game, 0, "creature", profileOf(removal))).toContainEqual({ kind: "permanent", instanceId: nemesis.instance_id });
  });

  it("returns Reincarnation's chosen creature after it dies this turn", () => {
    const profile = profileOf(C13_REINCARNATION());
    expect(profile).toMatchObject({ fullyImplemented: true, effects: [{ kind: "reincarnation-setup" }], targetKind: "creature" });
    let game = readyToCast([C13_REINCARNATION(), BOLT()], [FOREST(), FOREST(), FOREST(), MOUNTAIN()], [], [BEAR()]);
    const bear = game.players[1]!.battlefield.find((permanent) => permanent.card.name === "Grizzly Bears")!;
    game = applyAction(game, 0, { type: "cast", cardId: "hand-0", targets: [{ kind: "permanent", instanceId: bear.instance_id }] });
    game = passUntil(game, (state) => state.stack.length === 0 && state.delayedDeathReturns.length === 1);
  ...
  it("takes temporary control of Mass Mutiny's selected creature and grants haste", () => {
  ...
  it("reuses the Tempting Offer primitive for copy tokens", () => {
    const profile = profileOf(C13_TEMPT_WITH_REFLECTIONS());
    expect(profile).toMatchObject({ fullyImplemented: true, effects: [{ kind: "tempting-offer" }], targetKind: "creature-you-control" });
    let game = readyToCast([C13_TEMPT_WITH_REFLECTIONS()], [ISLAND(), ISLAND(), ISLAND(), ISLAND(), BEAR()], [], [BEAR()]);
    const ownBear = game.players[0]!.battlefield.find((permanent) => permanent.card.name === "Grizzly Bears")!;
    game = applyAction(game, 0, { type: "cast", cardId: "hand-0", targets: [{ kind: "permanent", instanceId: ownBear.instance_id }] });
    game = passUntil(game, (state) => state.pendingChoice?.type === "optional-trigger");
  ...
  it("reuses Tempting Offer for graveyard reanimation", () => {
    const profile = profileOf(C13_TEMPT_WITH_IMMORTALITY());
    expect(profile).toMatchObject({ fullyImplemented: true, effects: [{ kind: "tempting-offer" }], targetKind: "none" });
    let game = readyToCast([C13_TEMPT_WITH_IMMORTALITY()], [SWAMP(), SWAMP(), SWAMP(), SWAMP(), SWAMP()], [], [BEAR()]);
    game = stage(game, 0, (player) => ({ graveyard: toHand(0, [BEAR(), BEAR()], "immortality-caster") }));
    game = stage(game, 1, (player) => ({ graveyard: toHand(1, [BEAR()], "immortality-opponent") }));
    game = applyAction(game, 0, { type: "cast", cardId: "hand-0" });
    game = passUntil(game, (state) => state.pendingChoice?.type === "optional-trigger");
    expect(game.players[0]!.battlefield.some((permanent) => permanent.card.name === "Grizzly Bears")).toBe(true);
    const offer = game.pendingChoice as Extract<GameState["pendingChoice"], { type: "optional-trigger" }>;
  ...
  it("resolves Cruel Ultimatum as a reusable compound primitive", () => {
    const profile = profileOf(C13_CRUEL_ULTIMATUM());
    expect(profile).toMatchObject({ fullyImplemented: true, effects: [{ kind: "cruel-ultimatum" }], targetKind: "opponent" });
    let game = readyToCast(
      [C13_CRUEL_ULTIMATUM()],
      [ISLAND(), SWAMP(), SWAMP(), SWAMP(), MOUNTAIN(), MOUNTAIN(), MOUNTAIN()],
      [BEAR(), BEAR(), BEAR()],
      [BEAR()]
    );
    game = stage(game, 0, (player) => ({ graveyard: toHand(0, [BEAR()], "cruel-graveyard") }));
    game = applyAction(game, 0, { type: "cast", cardId: "hand-0", targets: [{ kind: "player", seat: 1 }] });
    game = applyAction(game, 0, { type: "pass" });
    game = applyAction(game, 1, { type: "pass" });
    while (game.pendingChoice?.type === "discard-cards") {
      const discard = game.pendingChoice;
      game = applyAction(game, 1, { type: "choose-discard", sourceId: discard.sourceId, cardId: game.players[1]!.hand[0]!.instance_id });
    }
    expect(game.players[1]!.battlefield.some((permanent) => permanent.card.name === "Grizzly Bears")).toBe(false);
    expect(game.players[1]!.hand).toHaveLength(0);
    expect(game.players[1]!.life).toBe(35);
    expect(game.players[0]!.life).toBe(45);
    expect(game.players[0]!.hand.some((card) => card.name === "Grizzly Bears")).toBe(true);
    expect(game.players[0]!.graveyard.some((card) => card.name === "Cruel Ultimatum")).toBe(true);
  });
  ...
  it("puts a counter on Sprite Dragon after its controller casts a noncreature spell", () => {
    // CR 603.2, 603.6: the spell-cast event is filtered by noncreature type
    // and the resulting self-trigger reuses the source-counter primitive.
    const dragon = SPRITE_DRAGON();
    expect(profileOf(dragon)).toMatchObject({
      fullyImplemented: true,
      triggers: [{ event: "spell-cast", subject: "you", spellType: "noncreature", effect: { kind: "add-counter-source", counter: "+1/+1", amount: 1 } }]
    });
    let game = readyToCast([RITUAL()], [dragon, SWAMP(), MOUNTAIN()]);
    const source = game.players[0]!.battlefield.find((permanent) => permanent.card.name === "Sprite Dragon")!;
    game = applyAction(game, 0, { type: "cast", cardId: "hand-0" });
    game = passUntil(game, (state) => (state.players[0]!.battlefield.find((permanent) => permanent.instance_id === source.instance_id)?.counters["+1/+1"] ?? 0) === 1);
...
describe("activated abilities", () => {
  ...
  it("returns a creature exiled by Mistmeadow Witch at the next end step", () => {
    // CR 603.7, 400.7: the delayed return tracks the exiled card, not the
    // permanent that left the battlefield.
    expect(profileOf(MISTMEADOW_WITCH())).toMatchObject({
      fullyImplemented: true,
      activatedAbilities: [{ manaCost: { raw: "{2}{W}{U}" }, targetKind: "creature", effect: { kind: "exile-target-permanent-delayed-return" } }]
    });
    expect(profileOf(ROON_OF_THE_HIDDEN_REALM())).toMatchObject({
      fullyImplemented: true,
      activatedAbilities: [{ requiresTap: true, excludesSourceFromTargets: true, targetKind: "creature", effect: { kind: "exile-target-permanent-delayed-return" } }]
    });
    let game = readyOnBoard([MISTMEADOW_WITCH()], { hold: true });
    game = putOnBattlefield(game, 1, [BEAR()]);
    game = stage(game, 0, () => ({ manaPool: { W: 1, U: 1, B: 0, R: 0, G: 0, C: 2 } }));
    const source = permanentNamed(game, 0, "Mistmeadow Witch")!;
    const target = permanentNamed(game, 1, "Grizzly Bears")!;
    game = applyAction(game, 0, { type: "activate", sourceId: source.instance_id, abilityIndex: 0, targets: [{ kind: "permanent", instanceId: target.instance_id }] });
    game = passUntil(game, (state) => state.players[1]!.exile.some((card) => card.instance_id === target.card.instance_id));
    expect(game.players[1]!.exile.some((card) => card.instance_id === target.card.instance_id)).toBe(true);
    game = passUntil(game, (state) => state.players[1]!.battlefield.some((permanent) => permanent.card.instance_id === target.card.instance_id));
    expect(game.players[1]!.battlefield.some((permanent) => permanent.card.instance_id === target.card.instance_id)).toBe(true);
  });

  it("keeps Roon's another-target restriction on its delayed exile ability", () => {
    let game = readyOnBoard([ROON_OF_THE_HIDDEN_REALM()], { hold: true });
    game = putOnBattlefield(game, 1, [BEAR()]);
    ...
    expect(() => applyAction(game, 0, {
      ...
      targets: [{ kind: "permanent", instanceId: source.instance_id }]
    })).toThrow("Objetivo ilegal");
  });
  ...
  it("supports Magus of the Arena fighting a creature of its controller's choice", () => {
    ...
    game = putOnBattlefield(game, 1, [BEAR()]);
    game = stage(game, 0, () => ({ manaPool: { W: 0, U: 0, B: 0, R: 0, G: 0, C: 3 } }));
    ...
    expect(profile).toMatchObject({
      fullyImplemented: true,
      activatedAbilities: [{ manaCost: { raw: "{3}" }, requiresTap: true, effect: { kind: "compound" }, targetKinds: ["creature-you-control", "creature-opponent"] }]
    });
    const activation = legalActions(game, 0).find((entry) => entry.action.type === "activate" && entry.action.sourceId === source.instance_id);
    expect(activation).toMatchObject({ requiresTargets: ["creature-you-control", "creature-opponent"] });
    game = applyAction(game, 0, {
  ...
  it("reuses each-other-player sacrifice for Shattergang Brothers", () => {
    let game = readyOnBoard([C13_SHATTERGANG_BROTHERS(), TEST_BEAST()], { hold: true });
    game = putOnBattlefield(game, 1, [BEAR()]);
    game = stage(game, 0, () => ({ manaPool: { W: 0, U: 0, B: 1, R: 0, G: 0, C: 2 } }));
    const source = permanentNamed(game, 0, "Shattergang Brothers")!;
    const sacrifice = permanentNamed(game, 0, "Test Beast")!;
    const profile = profileOf(C13_SHATTERGANG_BROTHERS());
    expect(profile).toMatchObject({
      fullyImplemented: true,
      activatedAbilities: [
        { manaCost: { raw: "{2}{B}" }, sacrificesCreature: "any", effect: { kind: "each-other-player-sacrifice", permanentType: "Creature" } },
        { manaCost: { raw: "{2}{R}" }, sacrificesPermanent: { type: "Artifact" }, effect: { kind: "each-other-player-sacrifice", permanentType: "Artifact" } },
        { manaCost: { raw: "{2}{G}" }, sacrificesPermanent: { type: "Enchantment" }, effect: { kind: "each-other-player-sacrifice", permanentType: "Enchantment" } }
      ]
    });
    const activation = legalActions(game, 0).find((entry) => entry.action.type === "activate" && entry.action.sourceId === source.instance_id && entry.action.abilityIndex === 0);
    expect(activation).toBeDefined();
    game = applyAction(game, 0, { ...activation!.action, sacrificeId: sacrifice.instance_id } as Extract<import("./engine.js").GameAction, { type: "activate" }>);
    game = passUntil(game, (state) => state.stack.length === 0);
  ...
  it("implements Deadwood Treefolk vanishing and enters/leaves return", () => {
    const profile = profileOf(C13_DEADWOOD_TREEFOLK());
    expect(profile).toMatchObject({
      fullyImplemented: true,
      vanishingAmount: 3,
    ...
    let game = twoSeatGame([], []);
    game = putOnBattlefield(game, 0, [C13_DEADWOOD_TREEFOLK()], { sick: false });
    expect(permanentNamed(game, 0, "Deadwood Treefolk")!.counters.time).toBe(3);
  });

  it("generates exact two-mode Incendiary Command choices", () => {
    const profile = profileOf(C13_INCENDIARY_COMMAND());
    expect(profile).toMatchObject({ fullyImplemented: true, modalChoices: expect.any(Array) });
    expect(profile.modalChoices).toHaveLength(6);
    expect(profile.modalChoices.some((choice) => choice.effect.kind === "compound"
      && choice.effect.effects.some((effect) => effect.kind === "each-player-discard-and-draw-own"))).toBe(true);

    let game = twoSeatGame([], []);
    game = stage(game, 0, () => ({ hand: toHand(0, [C13_INCENDIARY_COMMAND()]), autoPass: false }));
    game = putOnBattlefield(game, 0, [MOUNTAIN(), MOUNTAIN(), MOUNTAIN(), MOUNTAIN(), MOUNTAIN()]);
    game = passUntil(game, (state) => state.step === "precombat-main" && state.prioritySeat === 0);
    game = applyAction(game, 0, { type: "cast", cardId: "hand-0", mode: 4 });
    game = passUntil(game, (state) => state.stack.length === 0);
    expect(game.players[0]!.graveyard.some((card) => card.name === "Incendiary Command")).toBe(true);
  });
  ...
  it("fires noncombat damage triggers", () => {
    expect(profileOf(CHANDRAS_SPITFIRE()).triggers[0]).toMatchObject({
      event: "deals-damage-to-player", subject: "opponent", optional: false,
      condition: { kind: "noncombat-damage" }, effect: { kind: "modify-source-creature", power: 3, toughness: 0 }
    });
    expect(profileOf(CHANDRAS_SPITFIRE()).fullyImplemented).toBe(true);
    let game = readyOnBoard([DAMAGE_TRIGGERER(), CHANDRAS_SPITFIRE()], { hold: true });
    const source = permanentNamed(game, 0, "Damage Triggerer")!;
    game = applyAction(game, 0, {
      type: "activate", sourceId: source.instance_id, abilityIndex: 0,
      targets: [{ kind: "player", seat: 1 }]
  ...
  it("checks the active opponent's hand for Davriel's upkeep trigger", () => {
    const profile = profileOf(DAVRIEL_ROGUE_SHADOWMAGE());
    expect(profile.triggers[0]).toMatchObject({
      event: "upkeep", subject: "opponent", condition: { kind: "event-player-hand-at-most", amount: 1 },
      effect: { kind: "damage-event-player", amount: 2 }
    });
    expect(profile.fullyImplemented).toBe(true);

    let game = readyOnBoard([DAVRIEL_ROGUE_SHADOWMAGE()], { hold: true });
    game = stage(game, 1, () => ({ hand: [] }));
