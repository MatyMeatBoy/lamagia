...
function parseActivatedAbility(line: string, index: number): ActivatedAbility | null {
  ...
  const sacrificeCreatures = /sacrifice\s+(two|three|four|five|\d+)\s+(?:([A-Za-z][A-Za-z'’-]*)\s+)?creatures\b/i.exec(costText);
  ...
  const leftovers = costText
    ...
    .replace(/sacrifice\s+(?:two|three|four|five|\d+)\s+(?:[A-Za-z][A-Za-z'’-]*\s+)?creatures\b/gi, "")
  ...
  return {
    ...
    ...(sacrificeCreatures ? { sacrificesCreatures: { amount: toNumber(sacrificeCreatures[1])!, ...(sacrificeCreatures[2] ? { subtype: sacrificeCreatures[2] } : {}) } } : {}),
