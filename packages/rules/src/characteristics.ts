...
export interface ActivatedAbility {
  ...
  /** Multiple cards chosen from the controller's hand as one activation cost. */
  readonly discardsCards?: { readonly amount: number };
...
export type SpellEffect =
  ...
  /** Sudden Spoiling: target player's creatures lose abilities and become a fixed base P/T until cleanup. */
...
function parseActivatedAbility(line: string, index: number): ActivatedAbility | null {
  ...
  const discardsCardsMatch = /discard\s+(two|three|four|five|\d+)\s+cards?\b/i.exec(costText);
  const discardsCard = !discardsCardsMatch && /discard\s+(?:a|one)\s+card\b/i.test(costText);
  ...
  if (/^Target artifact creature gains deathtouch and lifelink until end of turn$/i.test(text)) {
    return {
      effect: {
        kind: "compound",
        effects: [
          { kind: "grant-target-creature-keyword", keyword: "deathtouch" },
          { kind: "grant-target-creature-keyword", keyword: "lifelink" }
        ]
      },
