...
export interface ActivatedAbility {
  ...
  readonly sacrificesPermanent?: { readonly type: "Artifact" | "Enchantment" | "Land" | "Noncreature" | "Token"; readonly mode: "any" | "another" };
...
export const TRIGGER_EVENT_LABELS: Readonly<Record<TriggerEvent, string>> = {
  ...
  "life-gained": "habilidad de ganancia de vida",
  "life-lost": "habilidad de pérdida de vida"
};
...
export type TargetKind =
  ...
  | "card-in-your-graveyard" | "card-in-a-graveyard" | "creature-card-in-your-graveyard" | "creature-card-in-a-graveyard" | "artifact-card-in-your-graveyard" | "artifact-card-in-a-graveyard" | "enchantment-card-in-your-graveyard" | "enchantment-card-in-a-graveyard" | "land-card-in-a-graveyard" | `subtype:${string}` | "none";
...
function parseActivatedAbility(line: string, index: number): ActivatedAbility | null {
  ...
  const sacrificePermanent = /sacrifice\s+(another\s+)?(?:a\s+|an\s+)?(artifact|enchantment|land|noncreature\s+permanent|token)\b/i.exec(costText);
  ...
  const leftovers = costText
    ...
    .replace(/sacrifice\s+(?:another\s+|a\s+|an\s+)?(?:artifact|enchantment|land|noncreature\s+permanent|token)\b/gi, "")
