...
export type SpellEffect =
  ...
  | { readonly kind: "reanimate-target-creature-and-lose-mana-value" }
  ...
  | { readonly kind: "create-token" | "create-token-for-target-player"; readonly amount: number | "X" | "lands-you-control" | "creatures-you-control" | "creatures-on-battlefield" | "equipment-attached-to-source" | "creatures-died-this-turn" | "opponents-with-4-plus-cards"; readonly token: TokenDefinition; readonly statsFromAmount?: boolean }
...
function recognizeSentence(sentence: string): { effect: SpellEffect; target: TargetKind } | null {
  ...
  if (/^Put target creature card from your graveyard onto the battlefield\. You lose life equal to that creature's (?:mana value|converted mana cost)$/i.test(text)) {
    return { effect: { kind: "reanimate-target-creature-and-lose-mana-value" }, target: "creature-card-in-your-graveyard" };
  }
  ...
  if ((match = /^Target opponent creates (.+)$/i.exec(text))) {
    const targetToken = parseCreateToken(`Create ${match[1]!}`);
    if (targetToken?.kind === "create-token") {
      return { effect: { ...targetToken, kind: "create-token-for-target-player" }, target: "opponent" };
    }
  }
