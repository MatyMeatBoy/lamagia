...
function recognizeText(text: string): RecognizedText {
  ...
  return { effects, triggers: resolvedTriggers, activatedAbilities, modalChoices, targetKind, combatOnly: false, declareBlockersOnly: false, kickerCost, kickerCosts, entwineCost, graftAmount, devourAmount, hasUpkeepSacrificeDraw, kickedEffects, kickedKeywords, kickedEntersWithCounters, hideawayAmount, evokeCost, flashbackCost, echoCost, suspendAmount: null, suspendCost: null, miracleCost, unimplementedText, covered: unimplementedText.length === 0 };
}
 ...
 const staticPowerToughnessGrants = parseStaticPowerToughnessGrants(text);
  ...
  const profile: CardProfile = {
   ...
   staticPowerToughnessGrants,
    ...
    fullyImplemented: recognized.covered || ((doublesPlusOneCounters || doublesPlusOneCountersGlobally) && (doublesTokens || doublesTokensGlobally)),
