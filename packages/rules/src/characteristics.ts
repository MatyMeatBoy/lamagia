...
export interface ActivatedAbility {
  ...
  /** Number of creature cards exiled from one graveyard as an activation cost. */
  readonly exilesCreatureCardsFromSingleGraveyard?: number;
  /** Counters removed from the source as an activation cost. */
  readonly removeCounters?: readonly CounterCost[];
  readonly lifeCost: number;
...
function parseActivatedAbility(line: string, index: number): ActivatedAbility | null {
  ...
  const exilesCreatureCardsFromSingleGraveyardMatch = /exile\s+(two|three|four|five|\d+)\s+creature\s+cards?\s+from\s+a\s+single\s+graveyard\b/i.exec(costText);
  ...
  return {
    ...
    ...(exilesCreatureCardsFromSingleGraveyardMatch ? { exilesCreatureCardsFromSingleGraveyard: toNumber(exilesCreatureCardsFromSingleGraveyardMatch[1]!)! } : {}),
