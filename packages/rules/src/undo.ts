...
/** Fail closed: only a manual, cost-free mana activation's pool/tap delta.
...
export function isSafeManaUndo(before: GameState, after: GameState, seat: SeatId, action: GameAction): boolean {
  ...
  if (!player || !source || !ability || ability.lifeCost || ability.gainLife || ability.removeCounters?.length
  ...
  const normalized: GameState = { ...after, version: before.version, log: before.log,
    players: after.players.map(p => p.seat !== seat ? p : { ...p, manaPool: player.manaPool,
