...
export interface Permanent {
  ...
  readonly kickerOptions?: readonly number[];
...
export interface StackObject {
  ...
  readonly kicked?: boolean;
  /** Selected independent optional kicker indexes (CR 702.33). */
  readonly kickerOptions?: readonly number[];
  /** The spell was cast for its Overload cost (CR 702.96). */
...
export type GameEvent =
  ...
  /** Damage dealt to a player, independent of whether it had a permanent source. */
  | { readonly kind: "damage-received"; readonly seat: SeatId; readonly amount: number }
...
export type PendingChoice =
  ...
  | {
      /** Order of Succession's sequential creature choices (CR 701.10, 903.2). */
      ...
      readonly selections: readonly { chooser: SeatId; instanceId: string }[];
    }
  | {
...
export type GameAction =
  ...
  | { readonly type: "cast"; readonly cardId: string; readonly targets?: readonly Target[]; readonly variableValue?: number; readonly mode?: number; readonly kicked?: boolean; readonly kickerOptions?: readonly number[]; readonly overloaded?: boolean; readonly evoked?: boolean; readonly entwined?: boolean; readonly fromGraveyard?: boolean; readonly flashback?: boolean; readonly freeCast?: boolean; readonly payLifeCost?: boolean; readonly returnPermanentId?: string; readonly returnPermanentIds?: readonly string[]; readonly payReducedCost?: boolean; readonly giftPromised?: boolean; readonly sacrificeId?: string }
  ...
  | { readonly type: "activate"; readonly sourceId: string; readonly abilityIndex: number; readonly targets?: readonly Target[]; readonly sacrificeId?: string; readonly sacrificeIds?: readonly string[]; readonly tapId?: string; readonly discardCardId?: string; readonly discardCardIds?: readonly string[]; readonly exileCardId?: string; readonly exileCardIds?: readonly string[]; readonly returnPermanentIds?: readonly string[]; readonly variableValue?: number }
...
function putOntoBattlefield(state: GameState, seat: SeatId, card: GameCard, isCommander: boolean, forceTapped = false, kicked = false, evoked = false, castFromHand = false, commanderEntryCounters = 0, castSpentMana: readonly ManaType[] = [], additionalCounters: readonly CounterCost[] = [], castVariableValue = 0, kickerOptions: readonly number[] = []): GameState {
  ...
  const permanent: Permanent = {
    ...
    deathtouched: false,
    ...(kicked ? { kicked: true } : {}),
    ...(kickerOptions.length ? { kickerOptions } : {}),
    ...(evoked ? { evoked: true } : {}),
...
function triggerMatches(
  ...
  if (condition?.kind === "source-has-counter-at-least") {
    const source = findPermanent(state, watcher.instanceId);
    if (!source || (source.counters[condition.counter] ?? 0) < condition.amount) return false;
  }
  ...
  if (event.kind === "damage-received") {
    if (subject === "each-player") return true;
    if (subject === "you") return event.seat === watcher.controller;
    return subject === "opponent" && event.seat !== watcher.controller;
  }
...
function causeOf(state: GameState, event: GameEvent): string {
  ...
  switch (event.kind) {
    ...
    case "damage-received": return `${playerAt(state, event.seat).name} recibe ${event.amount} de daño`;
...
function raiseEvent(
  ...
  for (const watcher of watchers) {
    ...
    for (const [index, definition] of definitions.entries()) {
      ...
      if (definition.requiresKicker !== undefined && !(watcher.kickerOptions ?? []).includes(definition.requiresKicker)) continue;
...
function dealDamageToPlayer(
  ...
  next = raiseEvent(next, { kind: "damage-received", seat, amount });
...
function applyEffect(state: GameState, object: StackObject, effect: SpellEffect, targetIndex = 0): GameState {
  ...
  switch (effect.kind) {
    ...
    case "return-source-to-hand": {
      if (object.activated && object.sourcePermanentId) {
        const permanent = findPermanent(state, object.sourcePermanentId);
        if (permanent) return returnPermanentToOwnersHand(state, permanent);
      }
    ...
    case "damage-each-opponent-creature": {
      let next = state;
      const amount = effectAmount(effect.amount, object);
      for (const permanent of allPermanents(state)) {
        const profile = cardProfile(permanent.card);
        if (!isCreature(profile) || !opponentsOf(state, controller).includes(permanent.controller)) continue;
    ...
    case "damage-target-creature-and-controller": {
      ...
      const next = dealDamageToPermanent(state, target.instanceId, effectAmount(effect.creatureAmount, object), false, sourceName, cardProfile(object.card), { controller: object.controller, permanentId: object.sourcePermanentId });
      return dealDamageFromObject(next, controller, effectAmount(effect.controllerAmount, object), sourceName, object);
    }
    ...
    case "set-target-player-creatures-base-pt-remove-abilities": {
      ...
      const next = withPlayer(state, target.seat, (player) => ({
        ...
        battlefield: player.battlefield.map((permanent) => isCreature(cardProfile(permanent.card))
          ? {
              ...permanent,
              temporaryBasePowerToughness: { power: effect.power, toughness: effect.toughness },
              temporaryAbilitiesRemoved: true
            }
          : permanent)
      }));
    ...
    case "destroy-target-creature-no-regeneration": {
      const target = object.targets[targetIndex];
      if (!target || target.kind !== "permanent") return state;
      const permanent = findPermanent(state, target.instanceId);
      if (!permanent) return state;
      const marked = withPlayer(state, permanent.controller, (player) => ({
        ...player,
        battlefield: player.battlefield.map((candidate) => candidate.instance_id === permanent.instance_id
          ? { ...candidate, cantRegenerateUntilEndOfTurn: true }
          : candidate)
      }));
      const updated = findPermanent(marked, permanent.instance_id);
      return updated ? destroyPermanent(marked, updated) : marked;
    }
    ...
    case "gain-control-target-while-source-tapped": {
      ...
      return logged(next, controller, `${sourceName}: ${moved.card.name} queda bajo tu control mientras ${source.card.name} permanezca girada.`);
    }
    case "spinal-embrace": {
      const target = object.targets[targetIndex];
      if (target?.kind !== "permanent") return state;
    ...
    case "add-counter-source": {
      ...
      const updated = findPermanent(next, source.instance_id);
      return updated && (updated.counters[effect.counter] ?? 0) >= 3
        ? raiseEvent(next, { kind: "source-counter-threshold", permanentId: updated.instance_id, controller: updated.controller, card: updated.card })
        : next;
    }
    case "toggle-source-counter": {
      const sourceId = object.sourcePermanentId ?? object.card.instance_id;
      const source = findPermanent(state, sourceId);
      if (!source) return state;
      const current = source.counters[effect.counter] ?? 0;
      const amount = current >= 3 ? -1 : 1;
      const next = withPlayer(state, source.controller, (player) => ({
        ...player,
        battlefield: player.battlefield.map((permanent) => permanent.instance_id === source.instance_id
          ? { ...permanent, counters: { ...permanent.counters, [effect.counter]: current + amount } }
          : permanent)
      }));
      const updated = findPermanent(next, source.instance_id);
      return updated && amount > 0 && (updated.counters[effect.counter] ?? 0) >= 3
        ? raiseEvent(next, { kind: "source-counter-threshold", permanentId: updated.instance_id, controller: updated.controller, card: updated.card })
        : next;
    }
    case "sacrifice-source-then-destroy-all-nonland": {
      const sourceId = object.trigger?.sourcePermanentId ?? object.sourcePermanentId ?? object.card.instance_id;
      const source = findPermanent(state, sourceId);
      if (!source) return state;
    ...
    case "remove-source-counter-gain-life": {
      const sourceId = object.trigger?.sourcePermanentId ?? object.sourcePermanentId ?? object.card.instance_id;
      const source = findPermanent(state, sourceId);
      if (!source || (source.counters[effect.counter] ?? 0) < 1) return state;
      let next = withPlayer(state, source.controller, (player) => ({
        ...player,
        battlefield: player.battlefield.map((permanent) => permanent.instance_id === source.instance_id
          ? { ...permanent, counters: { ...permanent.counters, [effect.counter]: (permanent.counters[effect.counter] ?? 0) - 1 } }
      ...
      if (playerCantGainLife(next, source.controller)) return next;
      next = withPlayer(next, source.controller, (player) => ({ ...player, life: player.life + effect.amount }));
      return logged(raiseEvent(next, { kind: "life-gained", seat: source.controller, amount: effect.amount }), source.controller, `${sourceName}: retira un contador y gana ${effect.amount} vida.`);
    }
...
function resolveTop(state: GameState): GameState {
  ...
  if (profile.isPermanent) {
    ...
    next = putOntoBattlefield(next, object.controller, object.card, isCommander, false, Boolean(object.kicked), Boolean(object.evoked), castFromHand,
      ...
      object.spentMana ?? [], [...(object.additionalCounters ?? []), ...variableEntryCounters], object.variableValue, object.kickerOptions ?? []);
  ...
  const resolvingEffects = object.overloaded ? profile.overloadedEffects : profile.effects;
  const kickedReplaces = object.kicked && resolvingEffects.some((effect) => effect.kind === "create-copy-token");
  ...
  if (!resolvingEffects.length && !(object.kicked && profile.kickedEffects.length)) {
...
function seatsInDirection(state: GameState, start: SeatId, direction: "left" | "right"): SeatId[] {
  const order = [start];
  let current = start;
  while (order.length < state.players.length) {
    const next = nearestOpponentInDirection(state, current, direction);
    if (next === undefined || order.includes(next)) break;
    order.push(next);
    current = next;
  }
  return order;
...
function orderSuccessionOptions(state: GameState, order: readonly SeatId[], index: number): Permanent[] {
  ...
  const targetSeat = order[(index + 1) % order.length];
  if (targetSeat === undefined) return [];
  return playerAt(state, targetSeat).battlefield.filter((permanent) => isCreature(cardProfile(permanent.card)));
}

function finishOrderSuccession(
  state: GameState,
  source: StackObject,
  selections: readonly { chooser: SeatId; instanceId: string }[]
): GameState {
  let next: GameState = { ...state, pendingChoice: null };
  for (const selection of selections) {
    const permanent = findPermanent(next, selection.instanceId);
    if (permanent && permanent.controller !== selection.chooser) {
      next = changePermanentController(next, permanent, selection.chooser);
    }
  }
  return logged(next, source.controller, `${source.card.name}: each player gains control of the chosen creature.`);
}

function beginOrderSuccessionChoice(state: GameState, source: StackObject, order: readonly SeatId[], index = 0, selections: readonly { chooser: SeatId; instanceId: string }[] = []): GameState {
  if (order.length < 2 || index >= order.length) return finishOrderSuccession(state, source, selections);
...
function spellCostOf(profile: CardProfile, kicked: boolean, evoked: boolean, entwined = false, overloaded = false, kickerIndexes: readonly number[] = []): ManaCost {
  if (evoked && profile.evokeCost) return profile.evokeCost;
  if (overloaded && profile.overloadCost) return profile.overloadCost;
  const cost = profile.kickerOptions.length
    ? withKickerOptions(profile.cost!, kickerIndexes.map((index) => profile.kickerOptions[index]).filter((cost): cost is ManaCost => Boolean(cost)))
    : withKicker(profile.cost!, kicked ? profile.kickerCost : null);
...
function castableCard(state: GameState, seat: SeatId, card: GameCard, fromCommandZone: boolean, variableValue = 0, mode?: number, kicked = false, evoked = false, flashback = false, entwined = false, freeCast = false, payLifeCost = false, returnPermanentId?: string, payReducedCost = false, giftPromised = false, returnPermanentIds?: readonly string[], overloaded = false, kickerIndexes: readonly number[] = []): { legal: boolean; note?: string; targetKind?: Exclude<TargetKind, "none">; targetKinds?: readonly Exclude<TargetKind, "none">[] } {
  const player = playerAt(state, seat);
  const profile = cardProfile(card);
  ...
  const cost = payReducedCost && profile.payReducedCostInstead
    ...
    : spellCostOf(profile, kicked, evoked, entwined, overloaded, kickerIndexes);
  ...
  if (!flashback && kicked && !profile.kickerCost && !profile.kickerOptions.length) return { legal: false };
  if (!flashback && kickerIndexes.some((index) => index < 0 || index >= profile.kickerOptions.length)
    || new Set(kickerIndexes).size !== kickerIndexes.length) return { legal: false };
  if (!flashback && overloaded && !profile.overloadCost) return { legal: false };
...
export function legalActions(state: GameState, seat: SeatId): LegalAction[] {
  ...
  if (state.pendingChoice) {
    ...
    if (choice.type === "choose-order-creature") {
      for (const permanent of orderSuccessionOptions(state, choice.order, choice.index)) {
        actions.push({
          action: { type: "choose-order-creature", sourceId: choice.sourceId, permanentId: permanent.instance_id },
          label: `Choose ${permanent.card.name}`,
          note: `${choice.sourceCard.name}: choose a creature controlled by the next player.`
        });
      }
      return actions;
    }
  ...
  if (!splitSecondActive(state)) for (const card of player.hand) {
    ...
    const kickerSelections: (readonly number[] | undefined)[] = profile.kickerOptions.length
      ? Array.from({ length: 1 << profile.kickerOptions.length }, (_, mask) => profile.kickerOptions.map((_, index) => index).filter((index) => (mask & (1 << index)) !== 0))
      : [undefined];
    for (const variableValue of values) for (const mode of modes) for (const { kicked, overloaded, evoked, entwined } of variants) for (const kickerSelection of kickerSelections) for (const sacrifice of sacrificeOptions) {
      const multiKicked = Boolean(kickerSelection?.length);
      const effectiveKicked = kicked || multiKicked;
      if (entwined && mode !== modes[0]) continue;
      const selectedMode = entwined ? undefined : mode;
      const check = castableCard(state, seat, card, false, variableValue, selectedMode, effectiveKicked, evoked, false, entwined, false, false, undefined, false, false, undefined, overloaded, kickerSelection);
      ...
      actions.push({
        action: { type: "cast", cardId: card.instance_id, ...(profile.cost?.hasVariable ? { variableValue } : {}), ...(selectedMode === undefined ? {} : { mode: selectedMode }), ...(effectiveKicked ? { kicked: true } : {}), ...(kickerSelection?.length ? { kickerOptions: kickerSelection } : {}), ...(overloaded ? { overloaded: true } : {}), ...(evoked ? { evoked: true } : {}), ...(entwined ? { entwined: true } : {}), ...(sacrifice ? { sacrificeId: sacrifice.instance_id } : {}) },
        label: `${profile.cost?.hasVariable ? `Lanzar ${card.name} (X=${variableValue})` : `Lanzar ${card.name}`}${effectiveKicked ? ` (kicker${kickerSelection?.length ? ` ${kickerSelection.map((index) => profile.kickerOptions[index]!.raw).join(" + ")}` : ""})` : ""}${overloaded ? " (overload)" : ""}${evoked ? " (evocar)" : ""}${entwined ? " (entwine)" : ""}${sacrifice ? ` — Sacrifice ${sacrifice.card.name}` : ""}${modal ? ` — ${modal.text}` : ""}`,
        cardId: card.instance_id,
        manaValue: cardProfile(card).manaValue + (profile.cost?.hasVariable ? variableValue : 0) + (kickerSelection?.reduce((sum, index) => sum + profile.kickerOptions[index]!.manaValue, 0) ?? (kicked ? (profile.kickerCost?.manaValue ?? 0) : 0)) + (overloaded ? (profile.overloadCost?.manaValue ?? 0) : 0) + (entwined ? (profile.entwineCost?.manaValue ?? 0) : 0),
  ...
  for (const card of player.commandZone) {
    ...
    const overloads = profile.overloadCost ? [false, true] : [false];
    const entwinedOptions = profile.entwineCost && profile.modalChoices.length ? [false, true] : [false];
    for (const variableValue of values) for (const mode of modes) for (const kicked of kickers) for (const overloaded of overloads) for (const entwined of entwinedOptions) {
      if (entwined && mode !== modes[0]) continue;
      const selectedMode = entwined ? undefined : mode;
      const check = castableCard(state, seat, card, true, variableValue, selectedMode, kicked, false, false, entwined, false, false, undefined, false, false, undefined, overloaded);
      if (!check.legal) continue;
      const modal = entwined ? combinedModalChoice(profile) : selectedMode === undefined ? undefined : profile.modalChoices[selectedMode];
      actions.push({
        action: { type: "cast", cardId: card.instance_id, ...(profile.cost?.hasVariable ? { variableValue } : {}), ...(selectedMode === undefined ? {} : { mode: selectedMode }), ...(kicked ? { kicked: true } : {}), ...(overloaded ? { overloaded: true } : {}), ...(entwined ? { entwined: true } : {}) },
        label: `Lanzar comandante ${card.name}${tax ? ` (+${tax} impuesto)` : ""}${kicked ? " (kicker)" : ""}${overloaded ? " (overload)" : ""}${entwined ? " (entwine)" : ""}${profile.cost?.hasVariable ? ` (X=${variableValue})` : ""}${modal ? ` — ${modal.text}` : ""}`,
        cardId: card.instance_id,
        manaValue: cardProfile(card).manaValue + tax + (profile.cost?.hasVariable ? variableValue : 0) + (kicked ? (profile.kickerCost?.manaValue ?? 0) : 0) + (overloaded ? (profile.overloadCost?.manaValue ?? 0) : 0) + (entwined ? (profile.entwineCost?.manaValue ?? 0) : 0),
        ...(check.targetKind ? { requiresTarget: check.targetKind } : {}),
        ...(check.targetKinds ? { requiresTargets: check.targetKinds } : {}),
        ...(check.note ? { note: check.note } : {})
      });
  ...
  for (const permanent of player.battlefield) {
    ...
    for (const ability of activatedAbilitiesFor(state, permanent)) {
      ...
      const returnLandSets: readonly (readonly Permanent[])[] = ability.returnLands
        ? combinations(player.battlefield.filter((candidate) => isLand(cardProfile(candidate.card))), ability.returnLands)
        : [[]];
      const discardSets: readonly (readonly GameCard[])[] = ability.discardsCards
        ? combinations(player.hand, ability.discardsCards.amount)
        : ability.discardsCard ? player.hand.map((card) => [card]) : [[]];
      ...
      for (const sacrificeSet of sacrificeSets) for (const returnLandSet of returnLandSets) for (const tapCreature of tapCreatures) for (const discardSet of discardSets) for (const exileSet of exileSets) actions.push({
        action: { type: "activate", sourceId: permanent.instance_id, abilityIndex: ability.index,
          ...
          ...(discardSet.length === 1 ? { discardCardId: discardSet[0]!.instance_id } : {}),
          ...(discardSet.length > 1 ? { discardCardIds: discardSet.map((card) => card.instance_id) } : {}),
          ...(exileSet.length === 1 ? { exileCardId: exileSet[0]!.instance_id } : {}),
          ...(exileSet.length > 1 ? { exileCardIds: exileSet.map((card) => card.instance_id) } : {}),
          ...(returnLandSet.length ? { returnPermanentIds: returnLandSet.map((land) => land.instance_id) } : {}) },
        label: `${permanent.card.name}: ${ability.text.split(":").slice(1).join(":").trim() || ability.text}${ability.manaCost?.hasVariable ? ` (X=${variableValue})` : ""}${sacrificeSet.length ? ` — Sacrifice ${sacrificeSet.map((candidate) => candidate.card.name).join(", ")}` : ""}${returnLandSet.length ? ` — Return ${returnLandSet.map((land) => land.card.name).join(", ")}` : ""}${tapCreature ? ` — Tap ${tapCreature.card.name}` : ""}${discardSet.length ? ` — Discard ${discardSet.map((card) => card.name).join(", ")}` : ""}${exileSet.length ? ` — Exile ${exileSet.map((card) => card.name).join(", ")}` : ""}`,
        cardId: permanent.instance_id,
        ...(check.targetKind ? { requiresTarget: check.targetKind } : {}),
        ...(check.targetKinds ? { requiresTargets: check.targetKinds } : {}),
        note: ability.text
...
export function legalTargets(state: GameState, seat: SeatId, kind: Exclude<TargetKind, "none">, sourceProfile?: CardProfile): Target[] {
  ...
  if (kind === "card-in-your-graveyard" || kind === "card-in-a-graveyard" || kind === "creature-card-in-your-graveyard" || kind === "creature-card-in-a-graveyard" || kind === "artifact-card-in-your-graveyard" || kind === "artifact-card-in-a-graveyard" || kind === "enchantment-card-in-your-graveyard" || kind === "enchantment-card-in-a-graveyard" || kind === "permanent-card-in-your-graveyard" || kind === "permanent-card-in-a-graveyard" || kind === "legendary-creature-card-in-your-graveyard" || kind === "instant-or-sorcery-card-in-your-graveyard" || creatureGraveyardManaValue) {
    ...
    return sources.flatMap((player) => player.graveyard
      .filter((card) => kind === "card-in-your-graveyard"
        ...
        || (creatureGraveyardManaValue && isCreature(cardProfile(card)) && cardProfile(card).manaValue <= Number(creatureGraveyardManaValue[1]))
  ...
  const filtered = permanents.filter((permanent) => {
    ...
    if (kind === "creature" || kind === "creature-you-control" || kind === "creature-opponent" || kind === "creature-opponent-without-flying" || kind === "artifact-creature" || kind === "nonartifact-creature" || kind === "nonblack-creature" || kind === "nonartifact-nonblack-creature" || kind === "non-demon-creature" || kind === "nonlegendary-creature" || kind === "creature-with-flying" || kind === "creature-with-defender" || kind === "creature-with-deathtouch" || kind === "creature-with-lifelink" || kind === "creature-with-menace" || kind === "creature-with-haste" || kind === "creature-with-first-strike" || kind === "creature-with-double-strike" || kind === "creature-with-trample" || kind === "creature-with-vigilance" || kind === "creature-with-indestructible" || kind === "creature-with-hexproof" || kind === "creature-with-shroud" || kind === "creature-with-reach" || kind === "creature-power-at-least-5" || kind === "creature-power-at-most-4" || kind === "creature-toughness-at-least-4" || kind === "creature-toughness-at-most-4" || kind === "creature-power-toughness-total-at-most-5") {
      ...
      if ((kind === "creature-opponent" || kind === "creature-opponent-without-flying") && permanent.controller === seat) return false;
      if (kind === "creature-opponent-without-flying" && keywordOf(state, permanent, "flying")) return false;
      ...
      if (kind === "creature-power-toughness-total-at-most-5" && powerOf(permanent, state) + toughnessOf(permanent, state) > 5) return false;
      return true;
    }
...
function withKickerOptions(cost: ManaCost, options: readonly ManaCost[]): ManaCost {
  return options.reduce((total, option) => withKicker(total, option), cost);
}
...
function activatableAbility(
  ...
  if (ability.discardsCards && player.hand.length < ability.discardsCards.amount) return { legal: false };
...
function applyActivate(state: GameState, seat: SeatId, action: Extract<GameAction, { type: "activate" }>): GameState {
  ...
  if (ability.discardsCards) {
    const ids = action.discardCardIds ?? [];
    const hand = playerAt(state, seat).hand;
    if (ids.length !== ability.discardsCards.amount || new Set(ids).size !== ids.length) {
      throw new Error(`Debes elegir ${ability.discardsCards.amount} cartas para descartar.`);
    }
    discardCards = ids.map((id) => hand.find((card) => card.instance_id === id)).filter((card): card is GameCard => Boolean(card));
    if (discardCards.length !== ability.discardsCards.amount) throw new Error("Debes elegir cartas disponibles para descartar.");
  }
  let exiles: GameCard[] = [];
  ...
  if (ability.returnLands !== undefined) {
    const selectedIds = action.returnPermanentIds ?? [];
    const candidates = playerAt(state, seat).battlefield.filter((candidate) => isLand(cardProfile(candidate.card)));
    const selected = selectedIds.map((id) => candidates.find((candidate) => candidate.instance_id === id));
    if (selectedIds.length !== ability.returnLands || new Set(selectedIds).size !== selectedIds.length || selected.some((candidate) => !candidate)) {
      throw new Error(`Debes devolver ${ability.returnLands} tierras a sus propietarios.`);
    }
  ...
  for (const land of returnedLands) {
    const current = findPermanent(next, land.instance_id);
    if (current) next = returnPermanentToOwnersHand(next, current);
  }
  ...
  if (ability.discardsCards) {
    ...
    const selected = ids.map((id) => hand.find((card) => card.instance_id === id)).filter((card): card is GameCard => Boolean(card));
    if (selected.length !== ability.discardsCards.amount) throw new Error("Debes elegir cartas disponibles para descartar.");
    for (const card of selected) next = discardCard(next, seat, card);
    next = logged(next, seat, `${player.name} descarta ${selected.map((card) => card.name).join(", ")}.`);
  }
...
function applyCast(state: GameState, seat: SeatId, action: Extract<GameAction, { type: "cast" }>): GameState {
  ...
  const kickerOptions = action.kickerOptions ?? [];
  const kicked = Boolean(action.kicked || kickerOptions.length);
  ...
  const check = castableCard(state, seat, card, Boolean(fromCommand), action.variableValue ?? 0, action.mode, kicked, evoked, fromGraveyard, entwined, freeCast, payLifeCost, returnPermanentId, payReducedCost, giftPromised, returnPermanentIds, overloaded, kickerOptions);
  ...
  const spellCost = payReducedCost && profile.payReducedCostInstead
    ...
    : fromGraveyard
    ? withKicker(profile.flashbackCost!, entwined ? profile.entwineCost : null)
    : spellCostOf(profile, kicked, evoked, entwined, overloaded, kickerOptions);
  ...
  next = pushOnStack(next, seat, card, action.targets ?? [], Boolean(fromCommand), action.variableValue ?? 0, selectedEffect, kicked, evoked, fromGraveyard, commanderEntryCounters,
    paymentSpentTypes, payReducedCost, false, Boolean(payment.spentRestricted?.some((mana) => mana.restriction.makesSpellUncounterable)), sacrificedPower, overloaded, kickerOptions);
  next = queueWardChoice(next, next.stack.at(-1)!);
...
function applyChooseDirection(state: GameState, seat: SeatId, action: Extract<GameAction, { type: "choose-direction" }>): GameState {
  ...
  if (choice.effect?.kind === "order-of-succession") {
    ...
    return logged(applyEffect(directed, source, choice.effect), seat,
      `${choice.sourceCard.name}: direction chosen is ${action.direction}.`);
  }
  return logged(directed, seat, `${choice.sourceCard.name}: direction chosen is ${action.direction}.`);
}
...
function applyChooseOrderCreature(state: GameState, seat: SeatId, action: Extract<GameAction, { type: "choose-order-creature" }>): GameState {
  const choice = state.pendingChoice;
  if (!choice || choice.type !== "choose-order-creature" || choice.seat !== seat) throw new Error("You do not have an Order of Succession creature choice pending.");
  if (choice.sourceId !== action.sourceId) throw new Error("That Order of Succession choice is no longer pending.");
  const options = orderSuccessionOptions(state, choice.order, choice.index);
  const permanent = options.find((candidate) => candidate.instance_id === action.permanentId);
  if (!permanent) throw new Error("Choose a creature controlled by the next player.");
  const selections = [...choice.selections, { chooser: seat, instanceId: permanent.instance_id }];
  const source: StackObject = {
    id: choice.sourceId,
    controller: choice.order[0]!,
    card: choice.sourceCard,
    label: choice.sourceCard.name,
    targets: [],
    fromCommandZone: false,
  ...
  return logged(beginOrderSuccessionChoice({ ...state, pendingChoice: null }, source, choice.order, choice.index + 1, selections), seat,
    `${playerAt(state, seat).name} chooses ${permanent.card.name}.`);
}
