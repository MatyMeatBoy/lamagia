...
export interface Permanent {
  ...
  readonly kickerIndices?: readonly number[];
...
export interface TriggerInstance {
  ...
  /** Reincarnation delayed return data retained after the chosen creature dies. */
...
/** Reincarnation-style delayed death trigger, valid only through the current turn. */
...
export type PendingChoice =
  ...
  | {
      ...
      readonly wardSacrifice?: "creature" | "permanent" | "legendary-artifact-or-creature" | "two-permanents" | "three-nonland-permanents" | "food" | "permanent-mana-value-at-least-one";
      readonly wardDiscardRandom?: boolean;
      readonly wardDiscardOrPayCost?: ManaCost;
...
export type GameAction =
  ...
  | { readonly type: "choose-trigger"; readonly sourceId: string; readonly accept: boolean; readonly tapIds?: readonly string[]; readonly variableValue?: number; readonly discardCardId?: string; readonly sacrificeId?: string; readonly sacrificeIds?: readonly string[] }
...
function jelevaExiledCards(state: GameState, sourceId: string): readonly { readonly card: GameCard; readonly owner: SeatId }[] {
  return state.players.flatMap((player) => player.exile
    .filter((card) => card.exiledWithSourceId === sourceId)
    .map((card) => ({ card, owner: player.seat })));
}
...
export function manaSources(player: PlayerState, state?: GameState, sourceOptions: { readonly allowedRestrictions?: readonly ManaRestrictionKind[]; readonly includeConvoke?: boolean; readonly includeImprovise?: boolean } = {}): ManaSource[] {
  ...
  if (sourceOptions.includeConvoke) {
    for (const permanent of player.battlefield) {
      const profile = cardProfile(permanent.card);
      if (!isCreature(profile) || permanent.tapped) continue;
      const options = profile.colors.length ? [...profile.colors] as ManaType[] : ["C"] as ManaType[];
      sources.push({
        permanentId: permanent.instance_id,
        abilityIndex: -1,
        name: permanent.card.name,
...
function putOntoBattlefield(state: GameState, seat: SeatId, card: GameCard, isCommander: boolean, forceTapped = false, kicked = false, evoked = false, castFromHand = false, commanderEntryCounters = 0, castSpentMana: readonly ManaType[] = [], additionalCounters: readonly CounterCost[] = [], kickerIndices: readonly number[] = []): GameState {
  ...
  const permanent: Permanent = {
    ...
    deathtouched: false,
    ...(kicked ? { kicked: true } : {}),
    ...(kickerIndices.length ? { kickerIndices } : {}),
    ...(evoked ? { evoked: true } : {}),
...
function raiseEvent(
  ...
  for (const watcher of watchers) {
    ...
    for (const [index, definition] of definitions.entries()) {
      ...
      if (definition.requiresKickerIndex !== undefined && !watcher.kickerIndices?.includes(definition.requiresKickerIndex)) continue;
...
function applyEffect(state: GameState, object: StackObject, effect: SpellEffect, targetIndex = 0): GameState {
  ...
  switch (effect.kind) {
    ...
    case "jeleva-exile-top-spent-mana": {
      const sourceId = object.trigger?.sourcePermanentId ?? object.sourcePermanentId;
      const source = sourceId ? findPermanent(state, sourceId) : undefined;
      const amount = object.trigger?.eventPermanentManaSpent ?? source?.castSpentMana?.length ?? object.spentMana?.length ?? 0;
      ...
      return logged(next, controller, `${source?.card.name ?? object.card.name} exiles the top ${amount} card(s) of each library.`);
    }
    case "jeleva-cast-exiled":
      // The optional attack trigger opens its private choice in resolveTop.
      return state;
    case "exile-self":
    ...
    case "event-player-sacrifice-creature": {
      ...
      const creatures = playerAt(state, victimSeat).battlefield.filter((permanent) => isCreature(cardProfile(permanent.card)));
      if (!creatures.length) return state;
      // "of their choice" is represented deterministically for the rules
      // server; a player-facing choice can be layered on this same effect.
      const victim = [...creatures].sort((left, right) =>
      ...
      return logged(next, victimSeat, `${playerAt(next, victimSeat).name} sacrifica ${victim.card.name}.`);
    }
    ...
    case "each-player-discard-and-draw-same": {
      let next = state;
      for (const player of state.players) {
        const hand = playerAt(next, player.seat).hand;
        const amount = hand.length;
    ...
    case "cruel-ultimatum": {
      ...
      let next = victim ? movePermanentToZone(state, victim, "graveyard", true) : state;
    ...
    case "remember-target-creature-for-death-reanimate": {
      const target = object.targets[0];
      if (target?.kind !== "permanent") return state;
      const permanent = findPermanent(state, target.instanceId);
      if (!permanent || !isCreature(cardProfile(permanent.card))) return state;
    ...
    case "tap-or-untap-equipped-creature": {
      ...
      return withPlayer(state, equipment.controller, (player) => ({
        ...player,
        battlefield: player.battlefield.map((permanent) => permanent.instance_id === attachedId
          ? { ...permanent, tapped: !permanent.tapped } : permanent)
      }));
    }
    ...
    case "add-counter-source": {
      ...
      const source = findPermanent(state, sourceId);
      if (!source) return state;
      const previous = source.counters[effect.counter] ?? 0;
      ...
      return updated && previous < 3 && (updated.counters[effect.counter] ?? 0) >= 3
    ...
    case "toggle-source-counter": {
      ...
      const updated = findPermanent(next, source.instance_id);
      return updated && amount > 0 && current < 3 && (updated.counters[effect.counter] ?? 0) >= 3
        ? raiseEvent(next, { kind: "source-counter-threshold", permanentId: updated.instance_id, controller: updated.controller, card: updated.card })
        : next;
    }
    case "sacrifice-source-then-destroy-all-nonland": {
      ...
      for (const permanent of allPermanents(next)) {
        if (!isLand(cardProfile(permanent.card))) {
          const current = findPermanent(next, permanent.instance_id);
          if (current) next = destroyPermanent(next, current);
        }
      }
      return logged(next, source.controller, `${sourceName} sacrifica ${source.card.name} y destruye todos los permanentes que no son tierras.`);
    }
...
function resolveTop(state: GameState): GameState {
  ...
  if (object.trigger) {
    ...
    if (object.trigger.definition.effect.kind === "jeleva-cast-exiled") {
      const options = jelevaExiledCards(next, object.trigger.sourcePermanentId)
        .filter(({ card }) => {
          const candidate = cardProfile(card);
          return candidate.types.includes("Instant") || candidate.types.includes("Sorcery");
        })
        .map(({ card }) => card.instance_id);
      if (!options.length) return logged(next, object.controller, `${object.trigger.sourceCard.name}: no exiled instant or sorcery is available.`);
      return {
        ...next,
        pendingChoice: {
          type: "jeleva-cast-exiled",
          seat: object.controller,
          sourceId: object.trigger.id,
  ...
  if (profile.isPermanent) {
    ...
    next = putOntoBattlefield(next, object.controller, object.card, isCommander, false, Boolean(object.kicked), Boolean(object.evoked), castFromHand,
      ...
      object.spentMana ?? [], [...(object.additionalCounters ?? []), ...variableEntryCounters], object.kickerIndices ?? []);
...
function kickerCostsOf(profile: CardProfile): readonly ManaCost[] {
  return profile.kickerCosts.length ? profile.kickerCosts : profile.kickerCost ? [profile.kickerCost] : [];
}

function withKickers(cost: ManaCost, profile: CardProfile, kickerIndices: readonly number[]): ManaCost {
  return kickerIndices.reduce((current, index) => withKicker(current, kickerCostsOf(profile)[index] ?? null), cost);
}
...
function spellCostOf(profile: CardProfile, kicked: boolean, evoked: boolean, entwined = false, overloaded = false, kickerIndices: readonly number[] = kicked ? [0] : []): ManaCost {
  if (overloaded && profile.overloadCost) return profile.overloadCost;
  if (evoked && profile.evokeCost) return profile.evokeCost;
  const cost = withKickers(profile.cost!, profile, kickerIndices);
...
function castableCard(state: GameState, seat: SeatId, card: GameCard, fromCommandZone: boolean, variableValue = 0, mode?: number, kicked = false, evoked = false, flashback = false, entwined = false, freeCast = false, payLifeCost = false, returnPermanentId?: string, payReducedCost = false, giftPromised = false, overloaded = false, targetCount = 1, kickerIndices: readonly number[] = kicked ? [0] : []): { legal: boolean; note?: string; targetKind?: Exclude<TargetKind, "none">; targetKinds?: readonly Exclude<TargetKind, "none">[] } {
  ...
  const cost = payReducedCost && profile.payReducedCostInstead
    ...
    : spellCostOf(profile, kicked, evoked, entwined, overloaded, kickerIndices);
  ...
  if (!flashback && ((!kicked && kickerIndices.length > 0)
    || (kicked && (kickerIndices.length === 0
      || new Set(kickerIndices).size !== kickerIndices.length
      || !kickerIndices.every((index) => index >= 0 && index < availableKickerCosts.length))))) return { legal: false };
...
export function legalActions(state: GameState, seat: SeatId): LegalAction[] {
  ...
  if (state.pendingChoice) {
    ...
    if (choice.type === "choose-direction") {
      actions.push({ action: { type: "choose-direction", sourceId: choice.sourceId, direction: "left" }, label: "Choose left", note: `${choice.sourceCard.name}: choose the attack direction.` });
    ...
    if (choice.type === "optional-trigger") {
      ...
      if (choice.paymentBy === "opponent" && choice.wardDiscardRandom) {
        if (player.hand.length) {
          actions.push({
            action: { type: "choose-trigger", sourceId: choice.sourceId, accept: true },
            label: "Discard a random card to pay Ward",
            note: `${choice.sourceCard.name}: discard a random card to prevent the spell from being countered.`
          });
        }
        actions.push({
      ...
      if (choice.paymentBy === "opponent" && choice.wardDiscard) {
        ...
        if (choice.wardDiscardOrPayCost && planManaPayment(choice.wardDiscardOrPayCost, player, { state })) {
          actions.push({
            action: { type: "choose-trigger", sourceId: choice.sourceId, accept: true },
            label: `Pay ${choice.wardDiscardOrPayCost.raw} to pay Ward`,
            note: `${choice.sourceCard.name}: pay the alternative Ward cost instead of discarding.`
          });
        }
        return actions;
      }
      ...
      if (choice.paymentBy === "opponent" && choice.wardSacrifice) {
        const candidates = player.battlefield.filter((permanent) => {
          ...
          if (choice.wardSacrifice === "legendary-artifact-or-creature") return profile.supertypes.includes("Legendary") && (isCreature(profile) || isArtifact(profile));
          if (choice.wardSacrifice === "two-permanents") return true;
          if (choice.wardSacrifice === "three-nonland-permanents") return !isLand(profile);
          if (choice.wardSacrifice === "food") return profile.subtypes.includes("Food");
          return profile.manaValue >= 1;
        });
        const amount = choice.wardSacrifice === "two-permanents" ? 2 : choice.wardSacrifice === "three-nonland-permanents" ? 3 : 1;
        for (const selected of combinations(candidates, amount)) {
          const permanent = selected[0]!;
          actions.push({
            action: { type: "choose-trigger", sourceId: choice.sourceId, accept: true, ...(amount === 1 ? { sacrificeId: permanent.instance_id } : { sacrificeIds: selected.map((candidate) => candidate.instance_id) }) },
            label: `Sacrifice ${selected.map((candidate) => candidate.card.name).join(", ")} to pay Ward`,
    ...
    if (choice.type === "jeleva-cast-exiled") {
      for (const cardId of choice.optionIds) {
        const entry = jelevaExiledCards(state, choice.sourcePermanentId).find(({ card }) => card.instance_id === cardId);
        if (!entry) continue;
        ...
        actions.push({
          ...
          label: `Cast ${entry.card.name} without paying its mana cost`,
          cardId,
          ...(targetKind !== "none" ? { requiresTarget: targetKind } : {}),
          ...(targetKinds?.length ? { requiresTargets: targetKinds } : {}),
          note: `${choice.sourceCard.name}: cast one exiled instant or sorcery.`
        });
      }
      actions.push({
  ...
  if (!splitSecondActive(state)) for (const card of player.hand) {
    ...
    const kickerCosts = kickerCostsOf(profile);
    const kickerVariants: readonly number[][] = kickerCosts.length
      ? Array.from({ length: 2 ** kickerCosts.length }, (_, mask) => kickerCosts.map((_, index) => index).filter((index) => (mask & (1 << index)) !== 0))
      : [[]];
    const variants: { kicked: boolean; kickerIndices: readonly number[]; overloaded: boolean; evoked: boolean; entwined: boolean }[] = kickerVariants.map((kickerIndices) => ({ kicked: kickerIndices.length > 0, kickerIndices, overloaded: false, evoked: false, entwined: false }));
    if (profile.evokeCost) variants.push({ kicked: false, kickerIndices: [], overloaded: false, evoked: true, entwined: false });
    if (profile.entwineCost && profile.modalChoices.length) variants.push({ kicked: false, kickerIndices: [], overloaded: false, evoked: false, entwined: true });
    if (profile.overloadCost) variants.push({ kicked: false, kickerIndices: [], overloaded: true, evoked: false, entwined: false });
    for (const variableValue of values) for (const mode of modes) for (const { kicked, kickerIndices, overloaded, evoked, entwined } of variants) for (const sacrifice of sacrificeOptions) {
      if (entwined && mode !== modes[0]) continue;
      const selectedMode = entwined ? undefined : mode;
      const check = castableCard(state, seat, card, false, variableValue, selectedMode, kicked, evoked, false, entwined, false, false, undefined, false, false, overloaded, 1, kickerIndices);
      ...
      actions.push({
        action: { type: "cast", cardId: card.instance_id, ...(profile.cost?.hasVariable ? { variableValue } : {}), ...(selectedMode === undefined ? {} : { mode: selectedMode }), ...(kicked ? { kicked: true, kickerIndices } : {}), ...(overloaded ? { overloaded: true } : {}), ...(evoked ? { evoked: true } : {}), ...(entwined ? { entwined: true } : {}), ...(sacrifice ? { sacrificeId: sacrifice.instance_id } : {}) },
        label: `${profile.cost?.hasVariable ? `Lanzar ${card.name} (X=${variableValue})` : `Lanzar ${card.name}`}${kicked ? ` (kicker ${kickerIndices.map((index) => kickerCosts[index]!.raw).join(" + ")})` : ""}${overloaded ? " (overload)" : ""}${evoked ? " (evocar)" : ""}${entwined ? " (entwine)" : ""}${sacrifice ? ` — Sacrifice ${sacrifice.card.name}` : ""}${modal ? ` — ${modal.text}` : ""}`,
        cardId: card.instance_id,
        manaValue: cardProfile(card).manaValue + (profile.cost?.hasVariable ? variableValue : 0) + kickerIndices.reduce((sum, index) => sum + (kickerCosts[index]?.manaValue ?? 0), 0) + (entwined ? (profile.entwineCost?.manaValue ?? 0) : 0),
  ...
  for (const card of player.commandZone) {
    ...
    const kickerVariants: readonly number[][] = kickerCosts.length
      ...
      : [[]];
    const entwinedOptions = profile.entwineCost && profile.modalChoices.length ? [false, true] : [false];
    for (const variableValue of values) for (const mode of modes) for (const kickerIndices of kickerVariants) for (const entwined of entwinedOptions) {
      const kicked = kickerIndices.length > 0;
      if (entwined && mode !== modes[0]) continue;
      const selectedMode = entwined ? undefined : mode;
      const check = castableCard(state, seat, card, true, variableValue, selectedMode, kicked, false, false, entwined, false, false, undefined, false, false, false, 1, kickerIndices);
      if (!check.legal) continue;
      const modal = entwined ? combinedModalChoice(profile) : selectedMode === undefined ? undefined : profile.modalChoices[selectedMode];
      actions.push({
        action: { type: "cast", cardId: card.instance_id, ...(profile.cost?.hasVariable ? { variableValue } : {}), ...(selectedMode === undefined ? {} : { mode: selectedMode }), ...(kicked ? { kicked: true, kickerIndices } : {}), ...(entwined ? { entwined: true } : {}) },
        label: `Lanzar comandante ${card.name}${tax ? ` (+${tax} impuesto)` : ""}${kicked ? ` (kicker ${kickerIndices.map((index) => kickerCosts[index]!.raw).join(" + ")})` : ""}${entwined ? " (entwine)" : ""}${profile.cost?.hasVariable ? ` (X=${variableValue})` : ""}${modal ? ` — ${modal.text}` : ""}`,
        cardId: card.instance_id,
        manaValue: cardProfile(card).manaValue + tax + (profile.cost?.hasVariable ? variableValue : 0) + kickerIndices.reduce((sum, index) => sum + (kickerCosts[index]?.manaValue ?? 0), 0) + (entwined ? (profile.entwineCost?.manaValue ?? 0) : 0),
        ...(check.targetKind ? { requiresTarget: check.targetKind } : {}),
        ...(check.targetKinds ? { requiresTargets: check.targetKinds } : {}),
        ...(check.note ? { note: check.note } : {})
      });
...
export function legalTargets(state: GameState, seat: SeatId, kind: Exclude<TargetKind, "none">, sourceProfile?: CardProfile): Target[] {
  ...
  const filtered = permanents.filter((permanent) => {
    ...
    if (kind === "creature" || kind === "creature-you-control" || kind === "creature-opponent" || kind === "creature-opponent-without-flying" || kind === "tapped-creature" || kind === "nonartifact-creature" || kind === "nonblack-creature" || kind === "nonartifact-nonblack-creature" || kind === "non-demon-creature" || kind === "nonlegendary-creature" || kind === "creature-with-flying" || kind === "creature-with-defender" || kind === "creature-with-deathtouch" || kind === "creature-with-lifelink" || kind === "creature-with-menace" || kind === "creature-with-haste" || kind === "creature-with-first-strike" || kind === "creature-with-double-strike" || kind === "creature-with-trample" || kind === "creature-with-vigilance" || kind === "creature-with-indestructible" || kind === "creature-with-hexproof" || kind === "creature-with-shroud" || kind === "creature-with-reach" || kind === "creature-power-at-least-5" || kind === "creature-power-at-most-4" || kind === "creature-toughness-at-least-4" || kind === "creature-toughness-at-most-4" || kind.startsWith("creature-power-toughness-sum-at-most-") || kind.startsWith("creature-power-at-") || kind.startsWith("creature-toughness-at-") || kind.startsWith("creature-power-or-toughness-")) {
      ...
      const eitherCap = /^creature-power-or-toughness-(at-least|at-most)-(\d+)$/.exec(kind);
      if (eitherCap) {
        const power = powerOf(permanent, state);
        const toughness = toughnessOf(permanent, state);
        if (eitherCap[1] === "at-least" ? power < Number(eitherCap[2]) && toughness < Number(eitherCap[2]) : power > Number(eitherCap[2]) && toughness > Number(eitherCap[2])) return false;
      }
      return true;
    }
...
function applyCast(state: GameState, seat: SeatId, action: Extract<GameAction, { type: "cast" }>, manaAlreadyPaid = false): GameState {
  ...
  const kickerIndices = action.kickerIndices ?? (kicked ? [0] : []);
  ...
  const check = castableCard(state, seat, card, Boolean(fromCommand), action.variableValue ?? 0, action.mode, kicked, evoked, fromGraveyard, entwined, freeCast, payLifeCost, returnPermanentId, payReducedCost, giftPromised, overloaded, Math.max(1, requested.length), kickerIndices);
  ...
  if (profile.additionalCostSacrificeArtifact) {
    ...
    next = movePermanentToZone(next, chosen, "graveyard", true);
  ...
  next = pushOnStack(next, seat, card, action.targets ?? [], Boolean(fromCommand), action.variableValue ?? 0, selectedEffect, kicked, evoked, fromGraveyard, commanderEntryCounters,
    paymentSpentTypes, payReducedCost, false, Boolean(payment.spentRestricted?.some((mana) => mana.restriction.makesSpellUncounterable)), sacrificedPower, sacrificedManaValue, overloaded, kickerIndices);
...
function queueWardPayment(state: GameState, object: StackObject, remainingWardTargets?: readonly string[]): GameState {
  ...
  const wardId = candidates.find((instanceId) => {
    ...
    return Boolean(permanent && permanent.controller !== object.controller
      && (profile?.wardCost || profile?.wardLifeCost !== null && profile?.wardLifeCost !== undefined || profile?.wardLifeCostPower || profile?.wardDiscard || profile?.wardDiscardRandom || profile?.wardDiscardOrPayCost || profile?.wardDiscardTypes.length || profile?.wardSacrifice));
  });
  ...
  return {
    ...
    pendingChoice: {
      ...
      ...(profile.wardDiscardRandom ? { wardDiscardRandom: true } : {}),
      ...(profile.wardDiscardOrPayCost ? { wardDiscard: true, wardDiscardOrPayCost: profile.wardDiscardOrPayCost, manaCost: profile.wardDiscardOrPayCost } : {}),
...
function nearestOpponentInDirection(state: GameState, seat: SeatId, direction: "left" | "right"): SeatId | undefined {
  ...
  for (let offset = 1; offset < seats.length; offset += 1) {
    ...
    if (!playerAt(state, candidate).lost) return candidate;
  }
  return undefined;
}
...
function applyChooseTrigger(state: GameState, seat: SeatId, action: Extract<GameAction, { type: "choose-trigger" }>): GameState {
  ...
  if (choice.paymentBy === "opponent") {
    ...
    if (choice.wardDiscardRandom) {
      const hand = playerAt(next, seat).hand;
      if (!hand.length) throw new Error(`No tienes cartas para descartar por Ward de ${choice.sourceCard.name}.`);
      const rolled = nextRandom(next.rngState);
      const discarded = hand[Math.floor(rolled.value * hand.length)]!;
      next = { ...next, rngState: rolled.state };
      next = discardCard(next, seat, discarded);
      const wardTarget = choice.targets?.[0];
      const wardStackId = wardTarget?.kind === "spell" ? wardTarget.stackId : undefined;
      const wardSpell = wardStackId ? next.stack.find((entry) => entry.id === wardStackId) : undefined;
      if (wardSpell && choice.remainingWardTargets?.length) next = queueWardPayment(next, wardSpell, choice.remainingWardTargets);
      return logged(next, seat, `${playerAt(next, seat).name} descarta una carta al azar para pagar Ward de ${choice.sourceCard.name}.`);
    }
    if (choice.wardDiscard) {
      if (choice.wardDiscardOrPayCost && !action.discardCardId) {
        const plan = planManaPayment(choice.wardDiscardOrPayCost, playerAt(next, seat), { state: next });
        if (!plan) throw new Error(`No puedes pagar ${choice.wardDiscardOrPayCost.raw} por Ward de ${choice.sourceCard.name}.`);
        next = applyManaPlan(next, seat, plan);
        const payment = payCost(choice.wardDiscardOrPayCost, playerAt(next, seat).manaPool, { availableLife: playerAt(next, seat).life });
        if (!payment) throw new Error(`No se pudo pagar ${choice.wardDiscardOrPayCost.raw} por Ward de ${choice.sourceCard.name}.`);
        next = withPlayer(next, seat, (current) => ({ ...consumeManaPayment(current, payment) }));
        const wardTarget = choice.targets?.[0];
        const wardStackId = wardTarget?.kind === "spell" ? wardTarget.stackId : undefined;
        const wardSpell = wardStackId ? next.stack.find((entry) => entry.id === wardStackId) : undefined;
        if (wardSpell && choice.remainingWardTargets?.length) next = queueWardPayment(next, wardSpell, choice.remainingWardTargets);
        return logged(next, seat, `${playerAt(next, seat).name} paga ${choice.wardDiscardOrPayCost.raw} para pagar Ward de ${choice.sourceCard.name}.`);
      }
    ...
    if (choice.wardSacrifice) {
      const amount = choice.wardSacrifice === "two-permanents" ? 2 : choice.wardSacrifice === "three-nonland-permanents" ? 3 : 1;
      const selectedIds = amount === 1 ? (action.sacrificeId ? [action.sacrificeId] : []) : [...(action.sacrificeIds ?? [])];
      const selected = selectedIds.map((id) => playerAt(next, seat).battlefield.find((permanent) => permanent.instance_id === id));
      const valid = selected.length === amount && new Set(selectedIds).size === amount && selected.every((permanent) => {
        if (!permanent || permanent.controller !== seat) return false;
        const profile = cardProfile(permanent.card);
        if (choice.wardSacrifice === "creature") return isCreature(profile);
        if (choice.wardSacrifice === "legendary-artifact-or-creature") return profile.supertypes.includes("Legendary") && (isCreature(profile) || isArtifact(profile));
        if (choice.wardSacrifice === "three-nonland-permanents") return !isLand(profile);
        if (choice.wardSacrifice === "food") return profile.subtypes.includes("Food");
        if (choice.wardSacrifice === "permanent-mana-value-at-least-one") return profile.manaValue >= 1;
        return true;
      });
      if (!valid) {
        throw new Error(`Debes elegir un ${choice.wardSacrifice} que controles para pagar Ward de ${choice.sourceCard.name}.`);
      }
      for (const permanent of selected as Permanent[]) {
        next = movePermanentToZone(next, findPermanent(next, permanent.instance_id) ?? permanent, "graveyard", true);
      }
      const wardTarget = choice.targets?.[0];
      const wardStackId = wardTarget?.kind === "spell" ? wardTarget.stackId : undefined;
      const wardSpell = wardStackId ? next.stack.find((entry) => entry.id === wardStackId) : undefined;
      if (wardSpell && choice.remainingWardTargets?.length) next = queueWardPayment(next, wardSpell, choice.remainingWardTargets);
      return logged(next, seat, `${playerAt(next, seat).name} sacrifica ${selected.map((permanent) => permanent!.card.name).join(", ")} para pagar Ward.`);
    }
...
function applyChooseJelevaCast(state: GameState, seat: SeatId, action: Extract<GameAction, { type: "choose-jeleva-cast" }>): GameState {
  ...
  if (!choice || choice.type !== "jeleva-cast-exiled" || choice.seat !== seat || choice.sourceId !== action.sourceId) {
    throw new Error("No Jeleva cast choice is pending.");
  }
  if (!action.cardId) return logged({ ...state, pendingChoice: null }, seat, `${playerAt(state, seat).name} declines Jeleva's spell.`);
  if (!choice.optionIds.includes(action.cardId)) throw new Error("That card was not exiled with Jeleva.");
  const entry = jelevaExiledCards(state, choice.sourcePermanentId).find(({ card }) => card.instance_id === action.cardId);
  if (!entry) throw new Error("That exiled spell is no longer available.");
  const profile = cardProfile(entry.card);
  if (!profile.types.includes("Instant") && !profile.types.includes("Sorcery")) throw new Error("Jeleva can cast only an instant or sorcery.");
  let targets: readonly Target[] = action.targets ?? [];
  if (profile.targetKinds?.length) {
    ...
    if (targets.length !== profile.targetKinds.length || targets.some((target, index) => !legalTargets(state, seat, profile.targetKinds![index]!, profile).some((candidate) => JSON.stringify(candidate) === JSON.stringify(target)))) {
      throw new Error(`${entry.card.name} needs legal targets.`);
    }
    ...
    if (!targets.length || targets.some((target) => !allowed.some((candidate) => JSON.stringify(candidate) === JSON.stringify(target)))) {
      throw new Error(`${entry.card.name} needs a legal target.`);
    }
  }
  let next = withPlayer({ ...state, pendingChoice: null }, entry.owner, (player) => ({
    ...player,
    exile: player.exile.filter((card) => card.instance_id !== entry.card.instance_id)
  }));
  next = pushOnStack(next, seat, entry.card, targets, false, 0);
  next = queueWardPayment(next, next.stack.at(-1)!);
