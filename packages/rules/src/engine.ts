...
export interface Permanent {
  ...
  /** Kicker components paid for this permanent, by printed component index. */
...
export interface StackObject {
  ...
  readonly kicked?: boolean;
  /** Kicker components paid for this spell, by printed component index. */
  readonly kickerIndices?: readonly number[];
  readonly evoked?: boolean;
...
export type GameAction =
  ...
  | { readonly type: "cast"; readonly cardId: string; readonly targets?: readonly Target[]; readonly variableValue?: number; readonly mode?: number; readonly kicked?: boolean; readonly kickerIndices?: readonly number[]; readonly evoked?: boolean; readonly entwined?: boolean; readonly fromGraveyard?: boolean; readonly flashback?: boolean; readonly freeCast?: boolean; readonly payLifeCost?: boolean; readonly returnPermanentId?: string; readonly payReducedCost?: boolean; readonly giftPromised?: boolean; readonly sacrificeId?: string; readonly discardCardId?: string }
...
function counterAmountWithReplacements(state: GameState, permanent: Permanent, counter: string, amount: number): number {
  ...
  const doublers = allPermanents(state).filter((candidate) => cardProfile(candidate.card).doublesPlusOneCountersGlobally).length
    + playerAt(state, permanent.controller).battlefield
...
function spellCostOf(profile: CardProfile, kicked: boolean, evoked: boolean, entwined = false, kickerIndices: readonly number[] = kicked ? [0] : []): ManaCost {
...
function castableCard(state: GameState, seat: SeatId, card: GameCard, fromCommandZone: boolean, variableValue = 0, mode?: number, kicked = false, evoked = false, flashback = false, entwined = false, freeCast = false, payLifeCost = false, returnPermanentId?: string, payReducedCost = false, giftPromised = false, kickerIndices: readonly number[] = kicked ? [0] : []): { legal: boolean; note?: string; targetKind?: Exclude<TargetKind, "none">; targetKinds?: readonly Exclude<TargetKind, "none">[] } {
  ...
  const cost = payReducedCost && profile.payReducedCostInstead
    ...
    : spellCostOf(profile, kicked, evoked, entwined, kickerIndices);
  ...
  if (!flashback && kicked && !kickerCosts.length) return { legal: false };
  if (!flashback && ((!kicked && kickerIndices.length > 0)
    || (kicked && (kickerIndices.length === 0 || new Set(kickerIndices).size !== kickerIndices.length
      || !kickerIndices.every((index) => index >= 0 && index < kickerCosts.length))))) return { legal: false };
...
export function legalActions(state: GameState, seat: SeatId): LegalAction[] {
  ...
  if (!splitSecondActive(state)) for (const card of player.hand) {
    ...
    const kickerVariants: readonly number[][] = kickerCosts.length
      ...
      : [[]];
    const variants: { kicked: boolean; kickerIndices: readonly number[]; evoked: boolean; entwined: boolean }[] = kickerVariants.map((kickerIndices) => ({ kicked: kickerIndices.length > 0, kickerIndices, evoked: false, entwined: false }));
    if (profile.evokeCost) variants.push({ kicked: false, kickerIndices: [], evoked: true, entwined: false });
    if (profile.entwineCost && profile.modalChoices.length) variants.push({ kicked: false, kickerIndices: [], evoked: false, entwined: true });
    for (const variableValue of values) for (const mode of modes) for (const { kicked, kickerIndices, evoked, entwined } of variants) for (const sacrifice of sacrificeOptions) {
      if (entwined && mode !== modes[0]) continue;
      const selectedMode = entwined ? undefined : mode;
      const check = castableCard(state, seat, card, false, variableValue, selectedMode, kicked, evoked, false, entwined, false, false, undefined, false, false, kickerIndices);
      ...
      actions.push({
        action: { type: "cast", cardId: card.instance_id, ...(profile.cost?.hasVariable ? { variableValue } : {}), ...(selectedMode === undefined ? {} : { mode: selectedMode }), ...(kicked ? { kicked: true, kickerIndices } : {}), ...(evoked ? { evoked: true } : {}), ...(entwined ? { entwined: true } : {}), ...(sacrifice ? { sacrificeId: sacrifice.instance_id } : {}) },
  ...
  for (const card of player.commandZone) {
    ...
    for (const variableValue of values) for (const mode of modes) for (const kickerIndices of kickerVariants) for (const entwined of entwinedOptions) {
      ...
      if (entwined && mode !== modes[0]) continue;
      const selectedMode = entwined ? undefined : mode;
      const check = castableCard(state, seat, card, true, variableValue, selectedMode, kicked, false, false, entwined, false, false, undefined, false, false, kickerIndices);
      if (!check.legal) continue;
      const modal = entwined ? combinedModalChoice(profile) : selectedMode === undefined ? undefined : profile.modalChoices[selectedMode];
      actions.push({
...
function activatableAbility(
  ...
  if (ability.targetKinds?.length) {
    const legalForKind = (kind: Exclude<TargetKind, "none">) => legalTargets(state, seat, kind, sourceProfile)
      .filter((target) => !ability.excludesSourceFromTargets
        || target.kind !== "permanent"
        || target.instanceId !== permanent.instance_id);
    if (ability.targetKinds.some((kind) => !legalForKind(kind).length)) return { legal: false };
    ...
    if (ability.targetKinds.every((kind) => kind === ability.targetKinds![0])
      && legalForKind(ability.targetKinds[0]!).length < ability.targetKinds.length) return { legal: false };
  ...
  if (!legalTargets(state, seat, targetKind, sourceProfile)
    .some((target) => !ability.excludesSourceFromTargets
      || target.kind !== "permanent"
      || target.instanceId !== permanent.instance_id)) return { legal: false };
...
function applyCast(state: GameState, seat: SeatId, action: Extract<GameAction, { type: "cast" }>, manaAlreadyPaid = false): GameState {
  ...
  const check = castableCard(state, seat, card, Boolean(fromCommand), action.variableValue ?? 0, action.mode, kicked, evoked, fromGraveyard, entwined, freeCast, payLifeCost, returnPermanentId, payReducedCost, giftPromised, kickerIndices);
  ...
  next = pushOnStack(next, seat, card, action.targets ?? [], Boolean(fromCommand), action.variableValue ?? 0, selectedEffect, kicked, evoked, fromGraveyard, commanderEntryCounters,
    paymentSpentTypes, payReducedCost, false, Boolean(payment.spentRestricted?.some((mana) => mana.restriction.makesSpellUncounterable)), sacrificedPower, sacrificedManaValue, kickerIndices);
