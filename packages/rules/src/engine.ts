...
export type GameAction =
  ...
  | { readonly type: "cast"; readonly cardId: string; readonly targets?: readonly Target[]; readonly variableValue?: number; readonly mode?: number; readonly fromGraveyard?: boolean }
...
function commanderColorIdentity(state: GameState, seat: SeatId): readonly ManaType[] {
  const player = playerAt(state, seat);
  const commanderIds = new Set(player.commanderIds);
  const cards = state.players.flatMap((candidate) => [
    ...candidate.commandZone, ...candidate.library, ...candidate.hand,
    ...candidate.graveyard, ...candidate.exile, ...candidate.battlefield.map((permanent) => permanent.card)
  ]);
  return [...new Set(cards
    .filter((card) => card.owner === seat && commanderIds.has(card.instance_id))
    .flatMap((card) => cardProfile(card).colorIdentity as ManaType[]))];
}

function manaOptionsForAbility(state: GameState, seat: SeatId, ability: ManaAbility): readonly ManaType[] {
  if (!ability.commanderIdentityOnly) return ability.produces;
  const identity = new Set(commanderColorIdentity(state, seat));
  return ability.produces.filter((mana) => identity.has(mana));
}
...
export function manaSources(player: PlayerState, state?: GameState): ManaSource[] {
  ...
  for (const permanent of player.battlefield) {
    ...
    for (const ability of profile.manaAbilities) {
      ...
      const options = state ? manaOptionsForAbility(state, player.seat, ability) : ability.produces;
...
function resolveTop(state: GameState): GameState {
  ...
  if (profile.effects.some(hasSelfShuffle)) return logged(next, object.controller, `${object.card.name} se baraja en la biblioteca de su propietario.`);
  return sendSpellToOwnerZone(next, object);
}
...
function castableCard(state: GameState, seat: SeatId, card: GameCard, fromCommandZone: boolean, variableValue = 0, mode?: number, flashback = false): { legal: boolean; note?: string; targetKind?: Exclude<TargetKind, "none"> } {
  const player = playerAt(state, seat);
  const profile = cardProfile(card);
...
export function legalActions(state: GameState, seat: SeatId): LegalAction[] {
  ...
  for (const card of player.hand) {
    ...
    if (!cost || !planManaPayment(cost, player, { state })) continue;
    actions.push({
  ...
  for (const permanent of player.battlefield) {
    ...
    for (const ability of profile.manaAbilities) {
      if (!canUseManaAbility(player, permanent, ability)) continue;
      const options = manaOptionsForAbility(state, seat, ability);
      if (!options.length) continue;
...
function pushOnStack(state: GameState, seat: SeatId, card: GameCard, targets: readonly Target[], fromCommandZone: boolean, variableValue: number, selectedEffect?: SpellEffect, flashback = false): GameState {
...
function applyActivateMana(state: GameState, seat: SeatId, action: Extract<GameAction, { type: "activate-mana" }>): GameState {
  ...
  if (!ability || !manaOptionsForAbility(state, seat, ability).includes(action.mana)) throw new Error("Esa habilidad de maná no existe.");
...
function applyCast(state: GameState, seat: SeatId, action: Extract<GameAction, { type: "cast" }>): GameState {
  ...
  const check = castableCard(state, seat, card, Boolean(fromCommand), action.variableValue ?? 0, action.mode, fromGraveyard);
  ...
  next = pushOnStack(next, seat, card, action.targets ?? [], Boolean(fromCommand), action.variableValue ?? 0, selectedEffect, fromGraveyard);
...
function applyChooseTrigger(state: GameState, seat: SeatId, action: Extract<GameAction, { type: "choose-trigger" }>): GameState {
  ...
  if (choice.manaCost) {
    const plan = planManaPayment(choice.manaCost, playerAt(next, seat), { state: next });
    if (!plan) throw new Error(`No tienes maná suficiente para pagar ${choice.manaCost.raw}.`);
    next = applyManaPlan(next, seat, plan);
    const payment = payCost(choice.manaCost, playerAt(next, seat).manaPool, { availableLife: playerAt(next, seat).life });
    if (!payment) throw new Error(`No se pudo pagar ${choice.manaCost.raw}.`);
    next = withPlayer(next, seat, (current) => ({ ...current, manaPool: payment.remaining }));
  }
  const source: StackObject = {
    id: choice.sourceId,
